...
typedef BOOL(CALLBACK* UpdateLayeredWindowIndirectPROC)(
	HWND, UPDATELAYEREDWINDOWINFO*);
...
LPVOID __ptr_ins(LPVOID* ptr, INT nCount, INT* nIndexInsert, INT cbBlock,
				 LPVOID pNewItem);
...
LPVOID _struct_createfromaddr(LPVOID lpAddr, size_t Offset, INT sizeofstruct,
							  INT* nError);
...
const TCHAR Chjrmc[][4] = { L"",     L"元旦", L"情人", L"妇女", L"植树", L"愚人",
						   L"劳动", L"青年", L"护士", L"儿童", L"建党", L"建军",
						   L"教师", L"国庆", L"平安", L"圣诞", L"春节", L"元宵",
						   L"清明", L"端午", L"七夕", L"中秋", L"重阳", L"祭祖",
						   L"腊八", L"小年", L"除夕" };
const TCHAR Chjqmc[][4] = { L"",     L"冬至", L"小寒", L"大寒", L"立春",
						   L"雨水", L"惊蛰", L"春分", L"清明", L"谷雨",
						   L"立夏", L"小满", L"芒种", L"夏至", L"小暑",
						   L"大暑", L"立秋", L"处暑", L"白露", L"秋分",
						   L"寒露", L"霜降", L"立冬", L"小雪", L"大雪" };
const TCHAR ChDay[][4] = {
	L"",     L"初一", L"初二", L"初三", L"初四", L"初五", L"初六", L"初七",
	L"初八", L"初九", L"初十", L"十一", L"十二", L"十三", L"十四", L"十五",
	L"十六", L"十七", L"十八", L"十九", L"二十", L"廿一", L"廿二", L"廿三",
	L"廿四", L"廿五", L"廿六", L"廿七", L"廿八", L"廿九", L"三十" };
const TCHAR ChMonth[][4] = { L"",     L"正月", L"二月", L"三月", L"四月",
							L"五月", L"六月", L"七月", L"八月", L"九月",
							L"十月", L"冬月", L"腊月" };
...
std::string UrlEncode(std::wstring url, BOOL notEncodeAlphanumeric = TRUE,
					  BOOL utf8 = TRUE, INT mode = 1);
std::wstring UrlEncodeW(std::wstring url, BOOL notEncodeAlphanumeric = TRUE,
						BOOL utf8 = TRUE, INT mode = 1);
std::wstring UrlDecode(const std::wstring& url, BOOL utf8 = TRUE);

HRESULT IsEffectRegistered(ID2D1Factory1* d2dFactory, const CLSID& effectID,
						   bool& result);
