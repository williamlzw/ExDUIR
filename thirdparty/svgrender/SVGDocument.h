...
namespace SVGNative
{
	...
	public:
		...
		void RoundedRect(float x, float y, float width, float height, float rx, float ry) ;
	...
	public:
		...
		{
			Image(GraphicStyleImpl& aGraphicStyle, std::set<std::string>& aClasses, std::shared_ptr<D2DSVGImageData> aImageData,
	...
	private:
		...
		std::unique_ptr<D2DSVGPath> ParseShape(xml::XMLNode* node, FillStyleImpl& fillStyle);//2025.01.14 尝试修复 渐变
	...
	private:
		...
		//-----------------------------------------------------------------------------------------
	public:
		std::unique_ptr<D2DSVGImageData> CreateImageData(const std::string& base64, ImageEncoding encoding);

		std::unique_ptr<D2DSVGPath> CreatePath();

		std::unique_ptr<Transform> CreateTransform(float a = 1.0, float b = 0.0, float c = 0.0, float d = 1.0, float tx = 0.0, float ty = 0.0);

		void Save(const GraphicStyle& graphicStyle);
		void Restore();

		void DrawPath(const D2DSVGPath& path, const GraphicStyle& graphicStyle, const FillStyle& fillStyle, const StrokeStyle& strokeStyle);
		void DrawImage(const D2DSVGImageData& image, const GraphicStyle& graphicStyle, const Rect& clipArea, const Rect& fillArea);
		Rect GetBounds(const D2DSVGPath& path, const GraphicStyle& graphicStyle, const FillStyle& fillStyle, const StrokeStyle& strokeStyle)
		{
