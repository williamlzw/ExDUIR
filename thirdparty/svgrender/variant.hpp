...
namespace std {
	...
	struct hash<std::detail::enabled_type<
		std::variant<Ts...>,
		std::lib::enable_if_t<std::lib::all<std::detail::hash::is_enabled<
		std::lib::remove_const_t<Ts>>()...>::value>>> {
...
#ifdef MPARK_GENERIC_LAMBDAS
					[](const auto& alt) {
						using alt_type = std::lib::decay_t<decltype(alt)>;
						using value_type = std::lib::remove_const_t<
	...
	private:
		...
		struct hasher {
			...
			inline std::size_t operator()(const Alt& alt) const {
				using alt_type = std::lib::decay_t<Alt>;
				using value_type =
					std::lib::remove_const_t<typename alt_type::value_type>;
				return hash<value_type>{}(alt.value);
			}
		};
#endif
