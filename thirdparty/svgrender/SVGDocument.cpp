...
bool SVGDocument::OpenSVGDocument(const char* s) {
  try {
    if (!mContext || !g_Ri.pD2Dfactory || !g_Ri.pWICFactory) return false;
...
HRESULT SVGDocument::GetUserStrokeColor(EXARGB* color, float* strokeWidth) {
  if (!color || !strokeWidth) return E_INVALIDARG;
...
void SVGDocument::TraverseSVGTree(XMLNode* rootNode) {
  if (!rootNode || strcmp(rootNode->GetName(), kSvgElem)) return;
  mRootNode = rootNode;
  ...
  if (!viewBoxAttr.found) {
    ...
    if (SVGStringParser::ParseListOfNumbers(viewBoxAttr.value, numberList) &&
        numberList.size() == 4)
...
float SVGDocument::ParseLengthFromAttr(const XMLNode* node,
                                       const char* attrName,
                                       LengthType lengthType, float fallback) {
...
// 优先使用标准 id 属性
  ...
  if (auto path = ParseShape(child, fillStyle)) {
    AddChildToCurrentGroup(
        std::unique_ptr<Graphic>(new Graphic(
            graphicStyle, classNames, fillStyle, strokeStyle, std::move(path))),
        std::move(idString));
  ...
  if (!strcmp(elementName, kGElem) ||
    ...
    if (hrefAttr.found) {
      ...
      if (dataURL.find(kDataUrlPngVal) == 0)
        ...
        return;
      imageData = CreateImageData(dataURL.substr(base64Offset), encoding);
    }
    ...
    if (imageData) {
      ...
      if (preserveAspectRatioAttr.found &&
        ...
        if (attrStringValues.size() == 2) meetOrSlice = attrStringValues[1];
      }
    ...
    auto hrefAttr = child->GetAttribute(kHrefAttr, kXlinkNS);
    if (!hrefAttr.found || !hrefAttr.value || hrefAttr.value[0] != '#') return;
    ...
    if (!isCloseToZero(x) || !isCloseToZero(y)) {
      if (!graphicStyle.transform) graphicStyle.transform = CreateTransform();
      graphicStyle.transform->Concat(1, 0, 0, 1, x, y);
    }

    std::string href{(hrefAttr.value + 1)};
    AddChildToCurrentGroup(
        std::make_shared<Reference>(graphicStyle, classNames, fillStyle,
                                    strokeStyle, std::move(href)),
        std::move(idString));
    ...
    if (attr.found) {
      ...
      if (SVGStringParser::ParseListOfNumbers(attr.value, numberList) &&
        ...
        graphicStyle.transform =
            CreateTransform(1, 0, 0, 1, -numberList[0], -numberList[1]);
    }
    ...
    for (auto clipPathChild = child->GetFirstNode(); clipPathChild != nullptr;
      ...
      // WebKit and Blink allow the clipping path if there is at least one valid
      // basic shape child.
      //
...
std::unique_ptr<D2DSVGPath> SVGDocument::ParseShape(XMLNode* child,
  ...
  if (fillStyle.hasFill)  // 有填充色  2025.01.14 尝试修复 渐变
  {
    if (std::holds_alternative<GradientImpl>(fillStyle.internalPaint)) {
      ...
      if (gradient.type ==
          GradientType::kLinearGradient)  // 如果是 线性渐变 则?
      {
        isLinear = true;
      } else if (gradient.type ==
                 GradientType::kRadialGradient)  // 如果是 径向渐变 则?
      {
        // isRadia = true;
      }
    }
  ...
  if (!strcmp(elementName, kRectElem)) {
    ...
    if (isCloseToZero(width) || isCloseToZero(height)) return nullptr;
    ...
    return path;
             ...
             !strcmp(elementName, kCircleElem)) {
    float rx{}, ry{};
    ...
    if (isCloseToZero(rx) || isCloseToZero(ry)) return nullptr;
    ...
    return path;
  } else if (!strcmp(elementName, kPolygonElem) ||
             !strcmp(elementName, kPolylineElem)) {
    auto attr = child->GetAttribute(kPointsAttr);
    if (!attr.found) return nullptr;
    ...
    if (size > 1) {
      ...
      i += 2;
      for (; i < size; i += 2) path->LineTo(numberList[i], numberList[i + 1]);
      if (!strcmp(elementName, kPolygonElem)) path->ClosePath();
    }
    ...
    return path;
      ...
      !strcmp(
          elementName,
          kPathElem))  // 转移出去处理,因为svg的path可能存在多个闭合曲线,而D2D一个ID2D1PathGeometry对象只能open一次
  {
...
GraphicStyleImpl SVGDocument::ParseGraphic(const XMLNode* node,
                                           FillStyleImpl& fillStyle,
...
static inline void AddDetectedProperty(const XMLNode* node,
  ...
  if (attr.found) propertySet.insert({propertyName, attr.value});
}
...
void SVGDocument::ParseFillProperties(FillStyleImpl& fillStyle,
  ...
  if (prop != iterEnd) {
    ...
    if (result == SVGDocument::Result::kSuccess) fillStyle.color = color;
  }
...
void SVGDocument::ParseStrokeProperties(StrokeStyleImpl& strokeStyle,
  ...
  if (prop != iterEnd) {
    ...
    if (strokeWidth == 0.0) strokeStyle.hasStroke = false;
  }
...
void SVGDocument::ParseGraphicsProperties(GraphicStyleImpl& graphicStyle,
  ...
  if (prop != iterEnd) {
    if (prop->second.compare(kNoneVal)) graphicStyle.display = false;
  }
  ...
  if (prop != iterEnd) {
    ...
    if (result == SVGDocument::Result::kSuccess) graphicStyle.stopColor = color;
  }
}
...
void SVGDocument::ParseColorStops(XMLNode* node, GradientImpl& gradient) {
  ...
  // has none.
  if (!colorStops.empty()) gradient.internalColorStops = colorStops;
}
...
void SVGDocument::ParseGradient(XMLNode* node) {
  ...
  if (attr.found) {
    ...
    if (it != mGradients.end()) gradient = it->second;
  }
  ...
  if (attr.found) mGradients.insert({attr.value, gradient});
}
...
void SVGDocument::Render(const char* id, const ColorMap& colorMap, float width,
                         float height) {
  // Referenced glyph identifiers shall be rendered as if they were contained in
  // a <defs> section under the root SVG element: Therefore, the referenced
  // shape/group should:
  // * ignore all styling and transforms on ancestors.
  // * inherit property values from the root SVG
  // element,https://docs.microsoft.com/en-us/typography/opentype/spec/svg#glyph-identifiers
  auto elementIter = mIdToElementMap.find(id);
  if (elementIter != mIdToElementMap.end())
    RenderElement(*elementIter->second, colorMap, width, height);
}

void SVGDocument::RenderElement(const Element& element,
                                const ColorMap& colorMap, float width_,
                                float height_) {
  if (width_ == 0) {
    width_ = width;
  }
...
#ifdef DEBUG_API
  for (auto const& bound : mBounds) sumBound = sumBound | bound;
#else
  sumBound = mBound;
#endif
  bound = sumBound;
  ...
  switch (element.Type()) {
    case ElementType::kReference: {
      ...
      if (refIt != mIdToElementMap.end()) {
        ApplyCSSStyle(reference.classNames, graphicStyle, fillStyle,
                      strokeStyle);
    ...
    case ElementType::kGraphic: {
      ...
      Rect bounds = GetBounds(*(graphic.path.get()), graphicStyle, fillStyle,
                              strokeStyle);
      if (!bounds.IsEmpty()) {
#ifdef DEBUG_API
    ...
    case ElementType::kImage: {
      ...
      Rect bounds =
          GetBounds(*path.get(), GraphicStyle{}, FillStyle{}, StrokeStyle{});
      if (!bounds.IsEmpty()) {
#ifdef DEBUG_API
        mBounds.push_back(bounds);
#else
    ...
    case ElementType::kGroup: {
      ...
      Save(graphicStyle);
      for (const auto& child : group.children) ExtractBounds(*child);
...
void SVGDocument::TraverseTree(const ColorMap& colorMap,
  ...
  switch (element.Type()) {
    ...
    case ElementType::kGraphic: {
      ...
      ResolvePaintImpl(colorMap, fillStyle.internalPaint, color,
                       fillStyle.paint);
      ResolvePaintImpl(colorMap, strokeStyle.internalPaint, color,
                       strokeStyle.paint);
      if (userFillColor && fillStyle.hasFill) {
        Color fillColor{
            {ExGetR(userFillColor) / 255.f, ExGetG(userFillColor) / 255.f,
             ExGetB(userFillColor) / 255.f, ExGetA(userFillColor) / 255.f}};
        fillStyle.paint = fillColor;
      }
      if (userStrokeColor && strokeStyle.hasStroke) {
        Color strokeColor{
            {ExGetR(userStrokeColor) / 255.f, ExGetG(userStrokeColor) / 255.f,
             ExGetB(userStrokeColor) / 255.f, ExGetA(userStrokeColor) / 255.f}};
        strokeStyle.paint = strokeColor;
        if (userStrokeWidth > 0) strokeStyle.lineWidth = userStrokeWidth;
      }
    ...
    case ElementType::kImage: {
      const auto& image = static_cast<const Image&>(element);
      ApplyCSSStyle(image.classNames, graphicStyle, fillStyle, strokeStyle);
      DrawImage(*(image.imageData.get()), graphicStyle, image.clipArea,
                image.fillArea);
      break;
    }
    case ElementType::kGroup: {
      ...
      Save(graphicStyle);
      for (const auto& child : group.children) TraverseTree(colorMap, *child);
...
void SVGDocument::ApplyCSSStyle(const std::set<std::string>&, GraphicStyleImpl&,
                                FillStyleImpl&, StrokeStyleImpl&) {}
void SVGDocument::ParseStyleAttr(const XMLNode*, std::vector<PropertySet>&,
                                 std::set<std::string>&) {}
...
std::unique_ptr<D2DSVGImageData> SVGDocument::CreateImageData(
    const std::string& base64, ImageEncoding encoding) {
  ...
  ThrowIfFailed(g_Ri.pWICFactory->CreateDecoderFromStream(
      stream.Get(), nullptr, WICDecodeMetadataCacheOnDemand, &imgDecoder));
  ...
  // Keep a BitmapSource and not an ID2D1Bitmap because the latter is tied to a
  // specific ID2D1RenderTarget.
...
std::unique_ptr<Transform> SVGDocument::CreateTransform(float a, float b,
                                                        float c, float d,
                                                        float tx, float ty) {
  return std::unique_ptr<D2DSVGTransform>(
      new D2DSVGTransform(a, b, c, d, tx, ty));
}
...
void SVGDocument::Save(const GraphicStyle& graphicStyle) {
  ...
  if (graphicStyle.clippingPath) {
    if (graphicStyle.clippingPath->transform)
      maskTransform = dynamic_cast<D2DSVGTransform*>(
                          graphicStyle.clippingPath->transform.get())
                          ->GetMatrix();
    const auto constPath =
        dynamic_cast<const D2DSVGPath*>(graphicStyle.clippingPath->path.get());
  ...
  mContext->PushLayer(
      D2D1::LayerParameters(D2D1::InfiniteRect(), maskPath.Get(),
                            D2D1_ANTIALIAS_MODE_PER_PRIMITIVE, maskTransform,
                            graphicStyle.opacity),
      layer.Get());

  // FIXME: May need to get applied before creating the layer to apply to
  // clipping path.
  D2D1_MATRIX_3X2_F transform;
  mContext->GetTransform(&transform);
  mContextTransform.push(transform);
  if (graphicStyle.transform) {
    D2D1_MATRIX_3X2_F gsTransform =
        dynamic_cast<D2DSVGTransform*>(graphicStyle.transform.get())
            ->GetMatrix();
...
ComPtr<ID2D1Brush> SVGDocument::CreateBrush(const Paint& paint) {
  ...
  if (std::holds_alternative<Color>(paint)) {
    ...
    mContext->CreateSolidColorBrush({color[0], color[1], color[2], color[3]},
                                    &solidColorBrush);
    ...
    for (const auto& stop : gradient.colorStops) {
      ...
      colorsStops.push_back(
          {stop.first, {color[0], color[1], color[2], color[3]}});
    }
    ComPtr<ID2D1GradientStopCollection> gradientStopCollection;
    mContext->CreateGradientStopCollection(
        colorsStops.data(), static_cast<UINT32>(colorsStops.size()),
        D2D1_GAMMA_2_2, D2DSpreadMethod(gradient.method),
    ...
    if (gradient.type == GradientType::kLinearGradient) {
      ...
      mContext->CreateLinearGradientBrush(
          ...
          gradientStopCollection.Get(), &linearGradientBrush);
      if (gradient.transform)
        linearGradientBrush->SetTransform(
            std::static_pointer_cast<D2DSVGTransform>(gradient.transform)
                ->GetMatrix());
      ...
      mContext->CreateRadialGradientBrush(
          D2D1::RadialGradientBrushProperties(D2D1::Point2F(realcx, realcy),
                                              D2D1::Point2F(realfx, realfy),
                                              realr, realr),
          gradientStopCollection.Get(), &radialGradientBrush);
      if (gradient.transform)
        radialGradientBrush->SetTransform(
            std::static_pointer_cast<D2DSVGTransform>(gradient.transform)
                ->GetMatrix());
...
void SVGDocument::DrawPath(const D2DSVGPath& renderPath,
                           const GraphicStyle& graphicStyle,
  ...
  if (strokeStyle.hasStroke) {
    ...
    g_Ri.pD2Dfactory->CreateStrokeStyle(
        D2D1::StrokeStyleProperties(
            lineCap, lineCap, lineCap, lineJoin, strokeStyle.miterLimit,
            D2D1_DASH_STYLE_CUSTOM, strokeStyle.dashOffset),
        strokeStyle.dashArray.data(),
        static_cast<UINT32>(strokeStyle.dashArray.size()), &d2dStrokeStyle);
    // auto hr =  //SVG_ASSERT(SUCCEEDED(hr)); //总是失败

    mContext->DrawGeometry(path.Get(), brush.Get(), strokeStyle.lineWidth,
                           d2dStrokeStyle.Get());
...
void SVGDocument::DrawImage(const D2DSVGImageData& image,
                            const GraphicStyle& graphicStyle,
  ...
  if (bitmapSource) {
    ...
    ThrowIfFailed(
        mContext->CreateBitmapFromWicBitmap(bitmapSource.Get(), &bitmap));
    ...
    if (clipArea.width < fillArea.width || clipArea.height < fillArea.height) {
      clipRect =
          D2D1::RectF(clipArea.x, clipArea.y, clipArea.x + clipArea.width,
    ...
    D2D1_RECT_F drawRect =
        D2D1::RectF(fillArea.x, fillArea.y, fillArea.x + fillArea.width,
