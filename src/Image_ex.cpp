...
IWICBitmap* _wic_convert(IWICBitmapSource* pBitmapSource, BOOL bFreeOld, INT* nError)
{
    ...
    *nError                             = g_Ri.pWICFactory->CreateFormatConverter(&pConverter);
    if (*nError == 0) {
        *nError = pConverter->Initialize(pBitmapSource,
        ...
        if (*nError == 0) {
            *nError = g_Ri.pWICFactory->CreateBitmapFromSource(
                pConverter, WICBitmapCacheOnDemand, &pBitmapConvert);
...
IWICBitmap* _wic_selectactiveframe(IWICBitmapDecoder* pDecoder, INT nIndex, INT* nError,
                                   D2D1_RECT_F* dest)
{
    ...
    if (framecount > 1) {
        if (_handle_validate(hImg, HT_IMAGE, (LPVOID*)&pImg, &nError)) {
            if (pImg->nMaxFrames_ > 1) {
                ...
                D2D1_RECT_F dest;
                IWICBitmap* pFrame =
                    _wic_selectactiveframe(pImg->pWicDecoder_, nIndex, &nError, &dest);
...
BOOL _img_lock(HEXIMAGE hImg, RECT* lpRectL, DWORD flags, INT PixelFormat,
               EX_BITMAPDATA* lpLockedBitmapData)   // FLAGS 1读 2写 3读写
{
    img_s* pImage = nullptr;
    INT    nError = 0;
    if (_handle_validate(hImg, HT_IMAGE, (LPVOID*)&pImage, &nError)) {
    ...
    if (_handle_validate(hImg, HT_IMAGE, (LPVOID*)&pImage, &nError)) {
        ...
        if (pBitmapData != 0) {
            ...
            if (_img_lock(hImg,
                          ...
                          2498570,
                          pBitmapData))   // PixelFormat32bppARGB
            {
...
HEXIMAGE _img_init(IWICBitmapSource* pBitmapSource, INT curframe, INT frames,
                   IWICBitmapDecoder* pDecoder, INT* nError)
{
...
HEXIMAGE _wic_create(INT width, INT height, GUID pFormat, INT* nError)
{
    ...
    *nError =
        g_Ri.pWICFactory->CreateBitmap(width, height, pFormat, WICBitmapCacheOnDemand, &pBitmap);
    if (*nError == 0) {
...
BOOL _img_createfrompngbits(LPVOID lpmem, HEXIMAGE* dstImg)
{
    ...
    nError                  = g_Ri.pWICFactory->CreateBitmapFromMemory(width,
                                                      height,
                                                      GUID_WICPixelFormat32bppPBGRA,
                                                      width * 4,
                                                      len,
                                                      (BYTE*)((size_t)lpmem + 3 * sizeof(INT)),
                                                      &pBitmapData);
    if (nError == 0) {
...
BOOL _img_createfrompngbits2(INT nWidth, INT nHeight, BYTE* pbBuffer, HEXIMAGE* dstImg)
{
    ...
    nError                  = g_Ri.pWICFactory->CreateBitmapFromMemory(
        nWidth, nHeight, GUID_WICPixelFormat32bppPBGRA, nWidth * 4, len, pbBuffer, &pBitmapData);
    if (nError == 0) {
        hImg = _img_init(pBitmapData, 0, 1, NULL, &nError);
    }
    Ex_SetLastError(nError);
...
BOOL _img_createfromstream(LPSTREAM lpStream, HEXIMAGE* phImg)
{
    ...
    HEXIMAGE           hImg     = 0;
    nError                      = g_Ri.pWICFactory->CreateDecoderFromStream(
        lpStream, NULL, WICDecodeMetadataCacheOnLoad, &pDecoder);
    if (nError == 0) {
...
BOOL _img_createfromfile(LPCWSTR lpwzFilename, HEXIMAGE* phImg)
{
    ...
    HEXIMAGE           hImg     = 0;
    INT                nError   = 0;
    nError                      = g_Ri.pWICFactory->CreateDecoderFromFilename(
        lpwzFilename, NULL, 2147483648, WICDecodeMetadataCacheOnLoad, &pDecoder);
    if (nError == 0) {
        hImg = _wic_init_from_decoder(pDecoder, &nError);
    }
    if (phImg) {
...
BOOL _img_copyrect(HEXIMAGE hImg, INT x, INT y, INT width, INT height, HEXIMAGE* phImg)
{
    ...
    if (_handle_validate(hImg, HT_IMAGE, (LPVOID*)&pImage, &nError)) {
        ...
        nError                          = g_Ri.pWICFactory->CreateBitmapFromSourceRect(
            pBitmapSource, x, y, width, height, &pIBitmap);
        if (nError == 0) {
...
BOOL _img_scale(HEXIMAGE hImg, INT width, INT height, HEXIMAGE* phImg)
{
    ...
    if (_handle_validate(hImg, HT_IMAGE, (LPVOID*)&pImage, &nError)) {
        ...
        if (pBitmapScaler) {
            nError = pBitmapScaler->Initialize(
                pBitmapSource, width, height, WICBitmapInterpolationModeLinear);
            if (nError == 0) {
...
BOOL _img_rotateflip(HEXIMAGE hImg, INT rfType, HEXIMAGE* phImg)
{
    ...
    if (_handle_validate(hImg, HT_IMAGE, (LPVOID*)&pImage, &nError)) {
        ...
        if (pBitmapFlipRotator) {
            nError =
                pBitmapFlipRotator->Initialize(pBitmapSource, (WICBitmapTransformOptions)rfType);
            if (nError == 0) {
            ...
            if (*nError == 0) {
                *nError = pIWICStream->InitializeFromIStream(pStream);
                if (*nError == 0) {
                    ...
                    *nError =
                        g_Ri.pWICFactory->CreateEncoder(GUID_ContainerFormatPng, NULL, &pEncoder);
                    if (*nError == 0) {
...
BOOL _img_createfromhbitmap(HBITMAP hBitmap, HPALETTE hPalette, BOOL fPreAlpha, HEXIMAGE* phImg)
{
    IWICBitmap* pBitmap = nullptr;
    HEXIMAGE    hImg    = 0;
    INT         nError  = g_Ri.pWICFactory->CreateBitmapFromHBITMAP(
        (HBITMAP)hBitmap,
        (HPALETTE)hPalette,
        fPreAlpha ? WICBitmapUsePremultipliedAlpha : WICBitmapUseAlpha,
        &pBitmap);
    if (nError == 0) {
...
BOOL _img_createfromcanvas(HEXCANVAS hCanvas, HEXIMAGE* dstImg)
{
    ...
    if (_handle_validate(hCanvas, HT_CANVAS, (LPVOID*)&pCanvas, &nError)) {
        ...
        // 创建临时位图
        if (SUCCEEDED(g_Ri.pD2DDeviceContext->CreateBitmap(
                D2D1::SizeU(nWidth, nHeight), NULL, 0, bp1, &pBitmap1))) {
            ...
            if (SUCCEEDED(pBitmap1->CopyFromBitmap(NULL, pCanvas->pBitmap_, NULL))) {
                ...
                if (SUCCEEDED(pBitmap1->Map(D2D1_MAP_OPTIONS_READ, &mrc))) {
                    // 创建一个空目标位图
                    if (SUCCEEDED(g_Ri.pWICFactory->CreateBitmap(nWidth,
                                                                 nHeight,
                                                                 GUID_WICPixelFormat32bppPBGRA,
                        ...
                        if (SUCCEEDED(pWICBitmap->Lock(&rect, WICBitmapLockWrite, &pLocker))) {
                            ...
                            if (SUCCEEDED(pLocker->GetDataPointer(&cbFrame, &aBits))) {
                                ...
                                for (UINT y = 0; y < nHeight; y++) {
                                    RtlMoveMemory(
                                        aBits + cbStride * y, mrc.bits + mrc.pitch * y, cbStride);
                                }
...
BOOL _img_mask(HEXIMAGE hImgDst, HEXIMAGE hImgSrc, DWORD nChannel, BOOL bBlackMask, HEXIMAGE* phImg)
{
                  ...
                  bdDst))   // PixelFormat32bppARGB
    {
        ...
        if (_img_scale(hImgSrc, bdDst->width, bdDst->height, &hImgTemp)) {
            ...
            if (_img_lock(
                    hImgTemp, NULL, WICBitmapLockRead, 925707, bdSrc))   // PixelFormat32bppARGB
            {
...
BOOL _img_paste(HEXIMAGE dstImg, HEXIMAGE srcImg, INT destX, INT destY, HEXIMAGE* phImg)
{
    ...
    if (FAILED(pSrcLock->GetStride(&srcStride)) || FAILED(pDestLock->GetStride(&destStride))) {
        pSrcLock->Release();
        pDestLock->Release();
        _img_destroy(hImg);
        return FALSE;
    ...
    for (UINT row = 0; row < srcHeight; row++) {
        ...
        for (UINT col = 0; col < srcWidth; col++) {
            ...
            pDestPixel[0] =
                static_cast<BYTE>(pSrcPixel[0] * srcAlpha + pDestPixel[0] * destAlpha);   // Blue
            pDestPixel[1] =
                static_cast<BYTE>(pSrcPixel[1] * srcAlpha + pDestPixel[1] * destAlpha);   // Green
            pDestPixel[2] =
                static_cast<BYTE>(pSrcPixel[2] * srcAlpha + pDestPixel[2] * destAlpha);   // Red
