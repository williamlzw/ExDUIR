...
void _treeview_calcitemmaxwidth(obj_s* pObj, EX_TREEVIEW_NODEITEM* item, INT* pWidth)
{
    ...
    if (item) {
        if (_obj_getextralong(pObj, TREEVIEW_LONG_HIMAGELIST)) {
            _imglist_size(
                (HEXIMAGELIST)_obj_getextralong(pObj, TREEVIEW_LONG_HIMAGELIST), &imgWidth, NULL);
        ...
        if (pObj->canvas_obj_) {
            while (item) {
                if (item->fExpand && item->pChildFirst) {
                    ...
                    FLOAT width = 0;
                    if (_canvas_calctextsize(pObj->canvas_obj_,
                                             pObj->hFont_,
                                             item->pwzText,
                                             -1,
                                             32,
...
void _treeview_updateitem(obj_s* pObj)
{
    ...
    if (currentWidth != width) {
        ...
        _obj_sendmessage(_obj_gethwnd(pObj),
                         pObj->hObj_,
                         pObj,
                         WM_SIZE,
                         0,
                        ...
                        EX_TREEVIEW_NODEITEM* insertAfter)
{
    ...
    if ((size_t)insertAfter == TREEVIEW_NODE_ROOT) {
        if (parent) {
            ...
            next = (EX_TREEVIEW_NODEITEM*)_obj_setextralong(
                pObj, TREEVIEW_LONG_NODEITEM, (size_t)item);
        }
        prev = item->pPrev;
    }
...
void _treeview_freeitem(obj_s* pObj, EX_TREEVIEW_NODEITEM* item, BOOL child, BOOL dellink)
{
    if (item) {
        ...
        _obj_dispatchnotify(_obj_gethwnd(pObj),
                            pObj,
                            pObj->hObj_,
                            pObj->id_,
                            TREEVIEW_EVENT_DELETEITEM,
                            0,
                            (size_t)item);
...
EX_TREEVIEW_NODEITEM* _treeview_insertitem(obj_s* pObj, EX_TREEVIEW_INSERTINFO* item)
{
    ...
    if (tvitem) {
        ...
        if (!item->fUpdateLater) {
            ...
            _obj_sendmessage(
                _obj_gethwnd(pObj), pObj->hObj_, pObj, LISTVIEW_MESSAGE_SETITEMCOUNT, count, 2, 0);
        }
    }
    return tvitem;
}
...
void _treeview_drawitem(obj_s* pObj, EX_NMHDR* lParam)
{
    ...
    if (item) {
        ...
        if (!_obj_dispatchnotify(_obj_gethwnd(pObj),
                                 pObj,
                                 pObj->hObj_,
                                 pObj->id_,
                                 TREEVIEW_EVENT_DRAWITEM,
                                 index,
                                 (size_t)ps)) {
            RECT rect;
...
EX_TREEVIEW_NODEITEM* _treeview_hittest(obj_s* pObj, POINT pt, INT* pType)
{
    ...
    if (cur > 0 && cur < Array_GetCount(pArray)) {
        ...
        if (pItem) {
            RECT rect = {0};
            _obj_baseproc(_obj_gethwnd(pObj),
                          pObj->hObj_,
                          pObj,
                          LISTVIEW_MESSAGE_GETITEMRECT,
                          pList->index_mouse_,
                          (size_t)&rect);
...
LRESULT CALLBACK _treeview_proc(HWND hWnd, HEXOBJ hObj, INT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    if (uMsg == WM_NOTIFY) {
        ...
        if (bDeleted && wParam) {
            ...
            INT count = _treeview_itemcount(
                (EX_TREEVIEW_NODEITEM*)_obj_getextralong(pObj, TREEVIEW_LONG_NODEITEM), &tmp);
            _obj_sendmessage(
                _obj_gethwnd(pObj), hObj, pObj, LISTVIEW_MESSAGE_SETITEMCOUNT, count, 2, 0);
    ...
    else if (uMsg == TREEVIEW_MESSAGE_SETITEM)   // lParam is EX_TREEVIEW_ITEMINFO
    {
        ...
        return (size_t)_treeview_hittest(
            pObj, {GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam)}, (INT*)wParam);
    }
        ...
        if (pItem && hitType == TREEVIEW_HITTYPE_ONITEMSTATEICON) {
            _obj_baseproc(
                hWnd, hObj, pObj, TREEVIEW_MESSAGE_EXPAND, !pItem->fExpand, (size_t)pItem);
...
void _treeview_register()
{
    ...
    _obj_register(ATOM_TREEVIEW,
                  OBJECT_STYLE_VISIBLE | OBJECT_STYLE_VSCROLL | OBJECT_STYLE_HSCROLL,
                  class_list.dwStyleEx,
                  class_list.dwTextFormat,
                  9 * sizeof(size_t),
                  NULL,
                  _treeview_proc,
                  class_list.dwFlags,
