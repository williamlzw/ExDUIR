...
void _listbuttonex_register() {
	...
	Ex_ObjRegister(L"Menubar", EOS_VISIBLE, EOS_EX_FOCUSABLE, DT_LEFT, cbObjExtra, NULL, 1, _listbuttonex_proc);
	Ex_ObjRegister(L"Toolbar", EOS_VISIBLE, EOS_EX_FOCUSABLE, DT_LEFT, 7 * sizeof(size_t), NULL, 2, _listbuttonex_proc);
	Ex_ObjRegister(L"Statusbar", EOS_VISIBLE, EOS_EX_FOCUSABLE, DT_LEFT, 7 * sizeof(size_t), NULL, 3, _listbuttonex_proc);
}
...
LRESULT CALLBACK _listbuttonex_proc(HWND hWnd, HEXOBJ hObj, INT uMsg, WPARAM wParam, LPARAM lParam) {
	...
	if (uMsg == WM_CREATE) {
		...
		Ex_ObjSetLong(hObj, LB_TYPE, nType);
		Ex_ObjSetLong(hObj, LB_CTCS, 0);
		Ex_ObjSetLong(hObj, LB_INDEX, 0);
		Ex_ObjSetLong(hObj, LB_HIMAGELIST, 0);
		array_s* hArr = Array_Create(0);
		if (hArr) {
			Array_SetExtra(hArr, hObj);
			Array_SetType(hArr, nType);
			Array_BindEvent(hArr, eae_delmember, _listbuttonex_arr_del);
			Ex_ObjSetLong(hObj, LB_ITEMINFO, (size_t)hArr);
		}
	}
	else if (uMsg == WM_DESTROY) {
		Array_Destroy((array_s*)Ex_ObjGetLong(hObj, LB_ITEMINFO));
		HEXIMAGELIST hImageList = (HEXIMAGELIST)Ex_ObjGetLong(hObj, LB_HIMAGELIST);
		if (hImageList) {
		...
		INT nType = Ex_ObjGetLong(hObj, LB_TYPE);
		if (nType == 1 || nType == 2) {
		...
		INT nCount = Ex_ObjGetLong(hObj, LB_CTCS);
		array_s* hArr = (array_s*)Ex_ObjGetLong(hObj, LB_ITEMINFO);
		EX_LISTBUTTON_ITEMINFO* pTR = (EX_LISTBUTTON_ITEMINFO*)Ex_MemAlloc(sizeof(EX_LISTBUTTON_ITEMINFO));
		if (pItemInfo->wzText) {
		...
		Ex_ObjSetLong(hObj, LB_CTCS, Array_GetCount(hArr));
		...
		EX_LISTBUTTON_ITEMINFO* pTR = (EX_LISTBUTTON_ITEMINFO*)Array_GetMember(hArr, (size_t)wParam);
		if (pTR != 0) {
			EX_LISTBUTTON_ITEMINFO* pItemInfo = (EX_LISTBUTTON_ITEMINFO*)lParam;
		...
		if (pTR != 0) {
			...
			if (pItemInfo->dwMask > 0) {
				...
				if ((pItemInfo->dwMask & 2) == 2) {
					if (pTR->wzText != L"") Ex_MemFree((LPVOID)pTR->wzText);
					if (pItemInfo->wzText) {
						pTR->wzText = StrDupW(pItemInfo->wzText);
					}
					else {
				...
				if ((pItemInfo->dwMask & 4) == 4) {
					if (pTR->wzTips != L"") Ex_MemFree((LPVOID)pTR->wzTips);
					if (pItemInfo->wzTips) {
						pTR->wzTips = StrDupW(pItemInfo->wzTips);
					}
					else {
				...
				if (nType < 3) {
					if ((pItemInfo->dwMask & 1) == 1 && (pItemInfo->dwMask & 2) == 2) {
						pTR->nWidth = _listbuttonex_itemWidth(hObj, nType, pTR->nImage, pTR->wzText);
					}
				}
				if (wParam != 0)
				{
		...
		if (Array_DelMember(hArr, (size_t)wParam) == TRUE) {
			Ex_ObjSetLong(hObj, LB_CTCS, nCount - 1);
			if ((int)lParam != 0) {
				_listbuttonex_update(hObj);
			}
			return 1;
	...
	else if (uMsg == LVM_SETIMAGELIST) {
		INT imgWidth = 0;
		INT imgHeight = 0;
		_imglist_size((HEXIMAGELIST)lParam, &imgWidth, &imgHeight);
		Ex_ObjSetLong(hObj, LB_HIMAGELIST, lParam);
		Ex_ObjSetLong(hObj, LB_HIMAGWIDTH, imgWidth);
		Ex_ObjSetLong(hObj, LB_HIMAGHEIGHT, imgHeight);
	}
	else if (uMsg == LBM_DOWNITEM) {
		if (IsMenu((HMENU)lParam)) {
			...
			if (_handle_validate(hObj, HT_OBJECT, (LPVOID*)&pObj, &nError)) {
				...
				Ex_TrackPopupMenu((HMENU)lParam, 0, lpRect.left + pObj->w_left_ + wParam, lpRect.top + pObj->w_bottom_ + 2, (size_t)hWnd, hObj, NULL, _listbuttonex_menu_proc, 0);
			}
		}
	}
	else if (uMsg == LBM_SELECTITEM) {
		POINT point = { 0 };
		POINT point2 = { 0 };
		GetCursorPos(&point);
		...
		array_s* hArr = (array_s*)Ex_ObjGetLong(hObj, LB_ITEMINFO);
		INT Index = Ex_ObjGetLong(hObj, LB_INDEX);
		...
		if (Heatindex > 0) {
			if (Heatindex != Index) {
				...
				pTR->dwState = 2;
				Ex_ObjSetLong(hObj, LB_INDEX, Heatindex);
...
void _listbuttonex_arr_del(array_s* hArr, INT nIndex, EX_LISTBUTTON_ITEMINFO* pvData, INT nType) {
	...
	if (nType == 1) {
		...
		if (pvData->wzTips != L"") Ex_MemFree((LPVOID)pvData->wzTips);
	}
	if (pvData->wzText != L"") Ex_MemFree((LPVOID)pvData->wzText);
	Ex_MemFree(pvData);
}
...
INT _listbuttonex_itemWidth(HEXOBJ hObj, INT nType, UINT nImage, LPCWSTR wzText) {
	...
	if (nType == 1) {
		if (_handle_validate(hObj, HT_OBJECT, (LPVOID*)&pObj, &nError)) {
			_canvas_calctextsize(pObj->canvas_obj_, pObj->hFont_, wzText, -1, 0, 0, pObj->c_right_, pObj->c_bottom_, &iWidth, &iHeight);
			nWidth = (UINT)iWidth + 15;
		}
	}
	else if (nType == 2) {
		if (nImage > 0) {
			nWidth = Ex_ObjGetLong(hObj, LB_HIMAGWIDTH) + 10;
		}
		if (wzText != L"") {
			if (_handle_validate(hObj, HT_OBJECT, (LPVOID*)&pObj, &nError)) {
				_canvas_calctextsize(pObj->canvas_obj_, pObj->hFont_, wzText, -1, 0, 0, pObj->c_right_, pObj->c_bottom_, &iWidth, &iHeight);
		...
		if (nCount > 0) {
			if (nType == 1) {
				...
				for (INT i = 1; i <= nCount; i++) {
					EX_LISTBUTTON_ITEMINFO* pTR = (EX_LISTBUTTON_ITEMINFO*)Array_GetMember((array_s*)Ex_ObjGetLong(hObj, LB_ITEMINFO), i);
					if (pTR->dwState > 0) {
						if (pTR->dwState == 1) {
				...
				HEXIMAGELIST hImageList = (HEXIMAGELIST)Ex_ObjGetLong(hObj, LB_HIMAGELIST);
				INT hImageWidth = Ex_ObjGetLong(hObj, LB_HIMAGWIDTH);
				INT hImageHeight = Ex_ObjGetLong(hObj, LB_HIMAGHEIGHT);
				...
				for (INT i = 1; i <= nCount; i++) {
					...
					if (pTR->nType == 0) {
						...
						_canvas_drawrect(ps.hCanvas, hBrush, pTR->nLeft + 6, 2, pTR->nLeft + 6, ps.uHeight - 4, 1, D2D1_DASH_STYLE_SOLID);
					}
					else {
						if (pTR->dwState == 1) {
							_brush_setcolor(hBrush, Ex_ObjGetColor(hObj, COLOR_EX_TEXT_HOVER));
						...
						if (pTR->nImage > 0) {
							...
							if (hImage != 0) {
								_canvas_drawimagerectrect(ps.hCanvas, hImage, nLeft + 5, hImageTop, nLeft + 5 + hImageWidth, hImageTop + hImageHeight, 0, 0, hImageWidth, hImageHeight, 255);
								nLeft += 5 + hImageWidth;
								nWidth -= hImageWidth;
							}
						}
						...
						if (pTR->wzText != L"") {
							...
							_canvas_drawtext(ps.hCanvas, Ex_ObjGetFont(hObj), crText, pTR->wzText, -1, DT_VCENTER | DT_SINGLELINE, nLeft, 0, nLeft + nWidth, ps.uHeight);
						}
				...
				for (INT i = 1; i <= nCount; i++) {
					EX_LISTBUTTON_ITEMINFO* pTR = (EX_LISTBUTTON_ITEMINFO*)Array_GetMember((array_s*)Ex_ObjGetLong(hObj, LB_ITEMINFO), i);
					_canvas_drawtext(ps.hCanvas, Ex_ObjGetFont(hObj), Ex_ObjGetColor(hObj, COLOR_EX_TEXT_NORMAL), pTR->wzText, -1, DT_VCENTER | DT_SINGLELINE | pTR->TextFormat, pTR->nLeft + 5, 0, pTR->nLeft + pTR->nWidth - 10, ps.uHeight);
					if (i < nCount) {
						_canvas_drawline(ps.hCanvas, hBrush, pTR->nLeft + pTR->nWidth, 0, pTR->nLeft + pTR->nWidth, ps.uHeight, 1, D2D1_DASH_STYLE_SOLID);
					}
				}
				_brush_destroy(hBrush);
			}
...
void _listbuttonex_mousemove(HEXOBJ hObj, INT nType, LPARAM lParam) {
	...
	//菜单条
	if (nType == 1) {
		//output(L"mousemov", pt.x, pt.y, L"hittest", Heatindex);
		if (Heatindex > 0) {
			if (Heatindex != Index) {
				if (Index > 0) {
