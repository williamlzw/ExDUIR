...
LRESULT _hook_oncreate(INT code, HWND hWnd, LPARAM lParam)
{
    ...
    EXATOM atomClass = (EXATOM)(LONG_PTR)(lpcs->lpszClass);
    auto hParent = lpcs->hwndParent;
    if (atomClass == ATOM_DIALOG)
    {
        wnd_s* pWndParent = nullptr;
        if (hParent)
        {
            auto hExDui = Ex_DUIFromWindow(hParent);
            _handle_validate(hExDui, HT_DUI, (LPVOID*)&pWndParent, 0);
        }
        else
            pWndParent = g_Li.MsgpWnd;
        if (pWndParent && g_Li.MsgpWnd)
        {
            mbp_s* pMsg = g_Li.MsgpWnd->lpMsgParams_;
            if (pMsg != 0)//如果是对话框 则pMsg为0  
            {
                ...
                Ex_DUIBindWindowEx(hWnd, pWndParent->base.hTheme_, style, (size_t)pMsg, pMsg->lpfnNotifyCallback_);
            }
            g_Li.MsgpWnd = 0;
        }
    }
...
void _msgbox_initdialog(HWND hWnd, wnd_s* pWnd, WPARAM wParam, LPARAM lParam)
{
    ...
    maxHeight = Ex_Scale(__get_int(pValue, 4)) + Ex_Scale(__get_int(pValue, 12)) + maxHeight +
                Ex_Scale(15) * 2;
    ...
    if (pfnCallback != 0) {
        _wnd_dispatch_notify(hWnd, pWnd, pWnd->base.hexdui_, -1, NM_INTDLG, 0, 0);
    }
    width  = Ex_Scale(80);
    height = Ex_Scale(24);
    GetWindowRect(hWnd, &rcWindow);
    ...
    top       = (FLOAT)maxHeight / g_Li.DpiY - __get_int(pValue, 12) +
          (FLOAT)(__get_int(pValue, 12) - 24) / 2;
    ...
    for (INT i = aryID.size() - 1; i >= 0; i--) {
        ...
        if (hObj != 0) {
            ...
            pObj->base.dwFlags_ = pObj->base.dwFlags_ | EOF_BMSGBOXCONTROL;
