...
void _sliderbar_register()
{
    ...
    Ex_ObjRegister(wzCls, OBJECT_STYLE_VISIBLE, OBJECT_STYLE_EX_FOCUSABLE | OBJECT_STYLE_EX_TABSTOP, NULL, 5 * sizeof(size_t), NULL, NULL, _sliderbar_proc);
}
...
void _sliderbar_paint(HEXOBJ hObj)
{
    ...
    if (Ex_ObjBeginPaint(hObj, &ps))
    {
        ...
        _canvas_fillellipse(ps.hCanvas, hBrush, Ex_Scale((FLOAT)(RC.left + RC.right)) / 2, Ex_Scale((FLOAT)(RC.top + RC.bottom)) / 2, Ex_Scale(Ex_ObjGetLong(hObj, SLIDERBAR_LONG_BLOCK_SIZE)), Ex_Scale(Ex_ObjGetLong(hObj, SLIDERBAR_LONG_BLOCK_SIZE)));
...
void _sliderbar_getrect(HEXOBJ hObj, RECT *rc)
{
    FLOAT value = NULL;
    Ex_ObjGetClientRect(hObj, rc);
    value = (FLOAT)(Ex_ObjGetLong(hObj, SLIDERBAR_LONG_MAX) - Ex_ObjGetLong(hObj, SLIDERBAR_LONG_MIN));
    value = (FLOAT)((Ex_ObjGetLong(hObj, SLIDERBAR_LONG_POS) - Ex_ObjGetLong(hObj, SLIDERBAR_LONG_MIN)) / value); // ' 得到百分比
    ...
    if ((Ex_ObjGetLong(hObj, OBJECT_LONG_STYLE) & SLIDERBAR_STYLE_VERTICAL) == SLIDERBAR_STYLE_VERTICAL)
    {
        if (Ex_ObjGetLong(hObj, SLIDERBAR_LONG_BLOCK_DIRECTION) == 1)
        {
            *rc = RECT{(long)(rc->right / 2 - Ex_ObjGetLong(hObj, SLIDERBAR_LONG_BLOCK_SIZE)),
                       ...
                       (long)((rc->bottom - Ex_ObjGetLong(hObj, SLIDERBAR_LONG_BLOCK_SIZE) * 2) - (rc->bottom - Ex_ObjGetLong(hObj, SLIDERBAR_LONG_BLOCK_SIZE) * 2) * value)};
        }
        else
        {
            *rc = RECT{(long)(rc->right / 2 - Ex_ObjGetLong(hObj, SLIDERBAR_LONG_BLOCK_SIZE)),
                       ...
                       (long)(rc->right / 2 + Ex_ObjGetLong(hObj, SLIDERBAR_LONG_BLOCK_SIZE)),
                       (long)((rc->bottom - Ex_ObjGetLong(hObj, SLIDERBAR_LONG_BLOCK_SIZE) * 2) * value + Ex_ObjGetLong(hObj, SLIDERBAR_LONG_BLOCK_SIZE) * 2)};
        }
    }
    else
    {
        if (Ex_ObjGetLong(hObj, SLIDERBAR_LONG_BLOCK_DIRECTION) == 1)
        {
            *rc = RECT{(long)((rc->right - Ex_ObjGetLong(hObj, SLIDERBAR_LONG_BLOCK_SIZE) * 2) + Ex_ObjGetLong(hObj, SLIDERBAR_LONG_BLOCK_SIZE) * 2 - (rc->right - Ex_ObjGetLong(hObj, SLIDERBAR_LONG_BLOCK_SIZE) * 2) * value),
                       ...
                       (long)((rc->right - Ex_ObjGetLong(hObj, SLIDERBAR_LONG_BLOCK_SIZE) * 2) - (rc->right - Ex_ObjGetLong(hObj, SLIDERBAR_LONG_BLOCK_SIZE) * 2) * value),
                       (long)(rc->bottom / 2 + Ex_ObjGetLong(hObj, SLIDERBAR_LONG_BLOCK_SIZE))};
        }
        else
        {
            *rc = RECT{(long)((rc->right - Ex_ObjGetLong(hObj, SLIDERBAR_LONG_BLOCK_SIZE) * 2) * value),
                       (long)(rc->bottom / 2 - Ex_ObjGetLong(hObj, SLIDERBAR_LONG_BLOCK_SIZE)),
                       (long)((rc->right - Ex_ObjGetLong(hObj, SLIDERBAR_LONG_BLOCK_SIZE) * 2) * value + Ex_ObjGetLong(hObj, SLIDERBAR_LONG_BLOCK_SIZE) * 2),
                       (long)(rc->bottom / 2 + Ex_ObjGetLong(hObj, SLIDERBAR_LONG_BLOCK_SIZE))};
        }
    }
}
...
FLOAT _sliderbar_getvalueofthepoint(HEXOBJ hObj, LPARAM lParam)
{
    ...
    if ((Ex_ObjGetLong(hObj, OBJECT_LONG_STYLE) & SLIDERBAR_STYLE_VERTICAL) == SLIDERBAR_STYLE_VERTICAL)
    {
        ...
        {
            value = ((short)HIWORD(lParam) - Ex_Scale(Ex_ObjGetLong(hObj, SLIDERBAR_LONG_BLOCK_SIZE))) / Ex_Scale((FLOAT)RC.bottom - Ex_ObjGetLong(hObj, SLIDERBAR_LONG_BLOCK_SIZE) * 2);
        }
    }
    else
    {
        ...
        {
            value = ((short)LOWORD(lParam) - Ex_Scale(Ex_ObjGetLong(hObj, SLIDERBAR_LONG_BLOCK_SIZE))) / Ex_Scale((FLOAT)RC.right - Ex_ObjGetLong(hObj, SLIDERBAR_LONG_BLOCK_SIZE) * 2);
        }
    }
    value = Ex_ObjGetLong(hObj, SLIDERBAR_LONG_MIN) + value * (Ex_ObjGetLong(hObj, SLIDERBAR_LONG_MAX) - Ex_ObjGetLong(hObj, SLIDERBAR_LONG_MIN)); // ' 得到百分比
