...
{
	EXHANDLE hHSB= pObj->objHScroll_;
	EXHANDLE hVSB= pObj->objVScroll_;
	RECT rcClient{ 0 };
...
int _lv_itemfrompos(obj_s* pObj, listview_s* pOwner, int x, int y, int* offsetPosX, int* offsetPosY)
{
	...
	*offsetPosY = 0;
	...
	if (PtInRect((RECT*)((size_t)pObj + offsetof(obj_s, c_left_)), pt))
	{
		...
		int offsetx = 0;
		int realtop = 0;
		if ((pObj->dwStyle_ & 列表风格_横向列表) == 列表风格_横向列表)
		{
...
void _lv_redrawitem(obj_s* pObj, listview_s* pOwner, int iItem)
{
	if (_lv_checkitem_view(pOwner, iItem))
	{
		RECT rcItem{ 0 };
		int nError = 0;
		_lv_rectfromiitem(pObj, pOwner, iItem, (pObj->dwStyle_ & 列表风格_横向列表) == 列表风格_横向列表, &rcItem);
...
size_t _lv_setitemcount(HWND hWnd, EXHANDLE hObj, obj_s* pObj, int nCount, size_t lParam)
{
	...
	_lv_updatesbvalue(hObj, pObj, pOwner, &pObj->c_left_);
	int nPosX = 0;
	int nPosY = 0;
	int nPage = 0;
	int nView = 0;
	...
	_lv_updateviewindex(hObj, pObj, pOwner, bHView, &pObj->c_left_);
	int nError = 0;
...
void _lv_onvscrollbar(HWND hWnd, EXHANDLE hObj, obj_s* pObj, UINT uMsg, size_t wParam, size_t lParam)
{
	...
	{
		nPos = Ex_ObjScrollGetPos(hSB, SB_CTL);
	}
	else {
		if (nCode == SB_PAGEUP)
		{
...
void _lv_btndown(HWND hWnd, EXHANDLE hObj, obj_s* pObj, UINT uMsg, size_t wParram, size_t lParam)
{
	...
	if (vCur)
	{
		if (_lv_queryitemstate(lpItems, iCur, 状态_禁止))
		{
			return;
		}
	}
	...
	int nCount = pOwner->count_items_;
	bool bSingleSel = !((pObj->dwStyle_ & 列表风格_允许多选) == 列表风格_允许多选);
	bool bShowAllwasy = ((pObj->dwStyle_ & 列表风格_始终显示选择项) == 列表风格_始终显示选择项);
	int iKey = 窗口_取功能键();
	...
	{
		if ((pObj->dwStyle_ & 列表风格_表项跟踪) == 列表风格_表项跟踪)
		{
			return;
		}
	}
		...
		if (bSingleSel)
		{
			if (vSelect)
			{
				if (!((pObj->dwStyle_ & 列表风格_始终显示选择项) == 列表风格_始终显示选择项) || vCur)
				{
					_lv_item_changestate(hWnd, hObj, pObj, pOwner, lpItems, iSelect, 状态_选择, true, 0, 0, 0);
					nSelects = nSelects - 1;
					return;
				}
			}
		...
		if (vCur)
		{
			...
			{
				_lv_item_changestate(hWnd, hObj, pObj, pOwner, lpItems, iCur, 状态_选择, false, uMsg, iCur, iSelect);
				nSelects = nSelects + 1;
			}
			else {
				//判断功能键
				if ((iKey & 2) != 0 && !bSingleSel)//shift
				{
					...
					{
						if (!_lv_queryitemstate(lpItems, i, 状态_选择))
						{
							if (!_lv_queryitemstate(lpItems, i, 状态_禁止))
							{
								_lv_item_changestate(hWnd, hObj, pObj, pOwner, lpItems, i, 状态_选择, false,i == iCur ? uMsg : 0, i, iSelect);
								nSelects = nSelects + 1;
							}
						}
					}
...
size_t _lv_reselect(HWND hWnd, EXHANDLE hObj, obj_s* pObj, listview_s* pOwner, int iItem, bool bShow)
{
	...
	if (nCount > 0)
	{
		...
		for (int i = 1; i < nCount + 1; i++)
		{
			if (_lv_queryitemstate(lpItems, i, 状态_选择))
			{
				_lv_item_changestate(hWnd, hObj, pObj, pOwner, lpItems, i, 状态_选择, true, 0, 0, 0);
			}
		}
		pOwner->count_selects_ = 0;
	}
	if (iItem != 0)
	{
		if (_lv_checkitem(pOwner, iItem))
		{
			...
			_lv_item_changestate(hWnd, hObj, pObj, pOwner, lpItems, iItem, 状态_选择, false, LVN_ITEMCHANGED, iItem, nCount);
			if (bShow)
			{
...
size_t _lv_showitem(HWND hWnd, EXHANDLE hObj, obj_s* pObj, int iItem, bool bCheck)
{
	...
	RECT rcItem{ 0 };
	_lv_rectfromiitem(pObj, pOwner, iItem, bHView, &rcItem);
	OffsetRect(&rcItem, -__get_int(lprc, 0), -__get_int(lprc, 4));
	EXHANDLE hSB = 0;
	int oPos = 0;
	int nPage = 0;
	int nLine = 0;
...
void _lv_mouseleave(HWND hWnd, EXHANDLE hObj, obj_s* pObj)
{
	...
	{
		_lv_item_changestate(hWnd, hObj, pObj, pOwner, lpItems, iLast, 状态_点燃, true, 0, 0, 0);
		pOwner->index_mouse_ = 0;
	}
}
...
void _lv_mousemove(HWND hWnd, EXHANDLE hObj, obj_s* pObj, size_t wParam, size_t lParam)
{
	...
	bool bSingelSelect = !((pObj->dwStyle_ & 列表风格_允许多选) == 列表风格_允许多选);
			...
			if (iCur != iSelect)
			{
				...
				if (vCur)
				{
					if (_lv_queryitemstate(lpItems, iCur, 状态_禁止))
					{
						...
						_lv_item_changestate(hWnd, hObj, pObj, pOwner, lpItems, iCur, 状态_选择, false, LVN_HOTTRACK, iCur, iSelect);
					}
				}
			...
			if (iCur != iLast)
			{
				...
				if (vCur)
				{
					if (_lv_queryitemstate(lpItems, iCur, 状态_禁止))
					{
						...
						_lv_item_changestate(hWnd, hObj, pObj, pOwner, lpItems, iCur, 状态_点燃, false, 0, 0, 0);
					}
				}
			}
		}
...
void _lv_drawitem(HWND hWnd, EXHANDLE hObj, obj_s* pObj, listview_s* pOwner, paintstruct_s ps, int iItem, RECT rcClip, RECT rcItem)
{
	...
	ecd.dwStyle = ps.dwStyle_;
	ecd.rcDraw.left = rcItem.left;
	ecd.rcDraw.top = rcItem.top;
	ecd.rcDraw.right = rcItem.right;
	ecd.rcDraw.bottom = rcItem.bottom;
	ecd.iItem = iItem;
	...
	if (Ex_ObjBeginPaint(hObj, &ps))
	{
		...
		for (int i = iStart; i < iEnd; i++)
		{
			if (_lv_checkitem_view(pOwner, i))
			{
				...
				RECT rcClip{ 0 };
				if (IntersectRect(&rcClip, (RECT*)&ps.p_left_, &rcItem))
				{
...
size_t _lv_proc(HWND hWnd, EXHANDLE hObj, UINT uMsg, size_t wParam, size_t lParam, obj_s* pObj)
{
	...
	else if (uMsg == LVM_GETITEMRECT)
	{
		if (_lv_checkitem((listview_s*)_obj_pOwner(pObj), wParam))
		{
			_lv_lprectfromiitem(pObj, (listview_s*)_obj_pOwner(pObj), wParam, (pObj->dwStyle_ & 列表风格_横向列表) == 列表风格_横向列表, (void*)lParam);
		}
	}
