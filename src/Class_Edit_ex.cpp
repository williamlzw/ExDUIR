...
public:
    ...
    BOOL TxCreateCaret(HBITMAP hbmp, INT xWidth, INT yHeight) {
        if (!FLAGS_CHECK(m_pOwner->pObj_->dwStyle_, EDIT_STYLE_HIDDENCARET)) {
            ...
            return CreateCaret(_obj_gethwnd(m_pOwner->pObj_), (HBITMAP)-1, xWidth,
                yHeight);
        }
        return FALSE;
    };
    ...
    BOOL TxSetCaretPos(INT x, INT y) {
        ...
        if (FLAGS_CHECK(pObj->dwState_, STATE_FOCUS)) {
            FLAGS_ADD(m_pOwner->flags_,
                EDIT_FLAG_BCARETCONTEXT | EDIT_FLAG_BSHOWCARET);
...
void _edit_init(HWND hWnd, HEXOBJ hObj, obj_s* pObj) {
    ...
    if (pOwner != 0) {
        ...
        _struct_createfromaddr(pOwner, offsetof(edit_s, prctext_), sizeof(RECT),
            &nError);
        _struct_createfromaddr(pOwner, offsetof(edit_s, prcinset_), sizeof(RECT),
            &nError);
        ...
        CreateTextServicesL =
            (CTSFunc)GetProcAddress(g_Ri.hRiched20, "CreateTextServices");
        if (pITH != 0) {
            ...
            if (CreateTextServicesL(NULL, pITH, &pIUnk) == 0) {
                ...
                pITS->TxSendMessage(EM_AUTOURLDETECT,
                    FLAGS_CHECK(pObj->dwStyle_, EDIT_STYLE_PARSEURL), 0,
                    &ret);
            }
        }
        if (g_Li.hMenuEdit == 0) {
...
void _edit_setpcf(obj_s* pObj, edit_s* pOwner, INT height) {
    ...
    if (pcf != 0) {
        ...
        DWORD dwMask = EDIT_SELECT_CHARFORMAT_BOLD | CFE_ITALIC |
            EDIT_SELECT_CHARFORMAT_UNDERLINE |
            EDIT_SELECT_CHARFORMAT_STRIKEOUT |
            EDIT_SELECT_CHARFORMAT_SIZE | EDIT_SELECT_CHARFORMAT_COLOR |
            EDIT_SELECT_CHARFORMAT_FACE | CFM_CHARSET |
...
void _edit_size(HWND hWnd, HEXOBJ hObj, obj_s* pObj) {
    ...
    if ((pObj->dwTextFormat_ & DT_SINGLELINE) == DT_SINGLELINE ||
        ...
        Ex_ObjScrollSetInfo(hObj, SCROLLBAR_TYPE_VERT, SIF_PAGE, 0, 0, height, 0,
            FALSE);
        Ex_ObjScrollSetInfo(hObj, SCROLLBAR_TYPE_HORZ, SIF_PAGE, 0, 0, width, 0,
            FALSE);
    }
    if (pOwner->its_) {
...
void CALLBACK _edit_timer_caret(HWND hWnd, UINT uMsg, UINT_PTR idEvent,
    DWORD dwTime) {
    INT nError = 0;
    ...
    if ((pOwner->flags_ & EDIT_FLAG_BCARETCONTEXT) == EDIT_FLAG_BCARETCONTEXT) {
        pOwner->flags_ =
            pOwner->flags_ - (pOwner->flags_ & EDIT_FLAG_BCARETCONTEXT);
        pOwner->flags_ = pOwner->flags_ | EDIT_FLAG_BCARETSHHOWED;
    }
    else {
        pOwner->flags_ =
            pOwner->flags_ | (EDIT_FLAG_BSHOWCARET | EDIT_FLAG_BCARETCONTEXT);
        pOwner->flags_ =
            pOwner->flags_ - (pOwner->flags_ & EDIT_FLAG_BCARETSHHOWED);
    }
    _obj_invalidaterect(
...
void _edit_txpaint(LPVOID pits, DWORD dwDrawAspect, LONG lindex,
    ...
    ((ITextServices*)pits)
        ->TxDraw(dwDrawAspect, lindex, pvAspect, (DVTARGETDEVICE*)ptd, hdcDraw,
            hicTargetDev, (LPCRECTL)lprcBounds, (LPCRECTL)lprcWBounds,
            (LPRECT)lprcUpdate, NULL, dwContinue, lViewId);
}

size_t _edit_scrollmsg(HEXOBJ hObj, obj_s* pObj, INT uMsg, WPARAM wParam,
    LPARAM lParam) {
    BOOL ret;
...
LRESULT _edit_sendmessage(obj_s* pObj, INT uMsg, WPARAM wParam, LPARAM lParam,
    BOOL* sOK) {
    LRESULT ret = 0;
    LPVOID pits = _edit_its(pObj);
    if (pits != nullptr) {
        auto lret =
            ((ITextServices*)pits)->TxSendMessage(uMsg, wParam, lParam, &ret);
        *sOK = lret == 0;
    }
    return ret;
}
...
void _edit_contextmenu(HWND hWnd, wnd_s* pWnd, HEXOBJ hObj, obj_s* pObj,
...
void _edit_command(obj_s* pObj, INT uMsg, WPARAM wParam, LPARAM lParam) {
    BOOL sOK;
    if (wParam == WM_UNDO || wParam == WM_CUT || wParam == WM_COPY ||
            ...
            if (bDrawBanner) {
                ...
                if (!((ps.dwState & STATE_FOCUS) != 0 &&
                    ...
                    _canvas_drawtext(ps.hCanvas, pObj->hFont_,
                        ...
                        dt, rcText->left, rcText->top, rcText->right,
                        rcText->bottom);
                    bDrawBanner = TRUE;
                }
            }
            ...
            if (!((pObj->dwStyle_ & EDIT_STYLE_HIDDENCARET) ==
                ...
                if (!((((edit_s*)ps.dwOwnerData)->flags_ & EDIT_FLAG_BSELECTED) ==
                    ...
                    if ((((edit_s*)ps.dwOwnerData)->flags_ & EDIT_FLAG_BCARETCONTEXT) ==
                        ...
                        if (!((((edit_s*)ps.dwOwnerData)->flags_ &
                            EDIT_FLAG_BCARETSHHOWED) == EDIT_FLAG_BCARETSHHOWED)) {
...
LRESULT CALLBACK _edit_proc(HWND hWnd, HEXOBJ hObj, INT uMsg, WPARAM wParam,
    LPARAM lParam) {
    BOOL bFree = FALSE;
    INT nError = 0;
    obj_s* pObj = nullptr;
        ...
        if (uMsg == WM_CREATE) {
            ...
            if (pits != nullptr) {
                ...
                if (SB == SB_THUMBTRACK)
                {
                    ...
                    ((ITextServices*)pits)->TxSendMessage(EM_SETSCROLLPOS, 0, (LPARAM)&scrollPos, &ret);
                }
                else if (SB == SB_THUMBPOSITION)
                {
                    if (uMsg == WM_VSCROLL)
                    {
                        scrollPos.y = Ex_ObjScrollGetPos(hObj, SB_VERT);
                    }
                    else
                    {
                        scrollPos.x = Ex_ObjScrollGetPos(hObj, SB_HORZ);
                    }
                    ((ITextServices*)pits)->TxSendMessage(EM_SETSCROLLPOS, 0, (LPARAM)&scrollPos, &ret);
                }
                else
            ...
            else if ((pObj->dwStyle_ & OBJECT_STYLE_HSCROLL) ==
                OBJECT_STYLE_HSCROLL) {
                uMsg = WM_HSCROLL;
            }
            auto zDelta = (short)HIWORD(wParam);
            if (uMsg != 0) {
                _edit_scrollmsg(hObj, pObj, uMsg, zDelta > 0 ? SB_LINEUP : SB_LINEDOWN,
                    0);
                return 1;
            }
        }
            ...
            if (!((pObj->dwStyle_ & EDIT_STYLE_HIDDENCARET) ==
                ...
                if ((pOwner->flags_ &
                    ...
                    (EDIT_FLAG_BSHOWCARET | EDIT_FLAG_BCARETCONTEXT)) {
                    pOwner->flags_ = pOwner->flags_ -
                        (pOwner->flags_ &
                            (EDIT_FLAG_BSHOWCARET | EDIT_FLAG_BCARETCONTEXT));

                    _obj_invalidaterect(
                        pObj, (RECT*)((size_t)pOwner + offsetof(edit_s, rcCaret_left_)),
                        &nError);
                }
            }
            BOOL ret = FALSE;
            ...
            if ((pObj->dwStyle_ & EDIT_STYLE_NUMERIC_LETTER) ==
                EDIT_STYLE_NUMERIC_LETTER) {
            ...
            _edit_contextmenu(hWnd, pObj->pWnd_, hObj, pObj, wParam,
            ...
            _edit_sendmessage(pObj, EM_GETOLEINTERFACE, 0, (LPARAM)&pRichEditOle,
                &bOK);
            if (bOK) {
...
DWORD CALLBACK _edit_stream_proc(DWORD_PTR dwCookie, LPBYTE pbBuff, LONG cb,
    LONG* pcb) {
    ...
    if (load) {
        ...
        ((EX_EDIT_STREAM*)dwCookie)->rtf =
            LPCWSTR((size_t)((EX_EDIT_STREAM*)dwCookie)->rtf + cb);
...
void CImageDataObject::InsertBitmap(IRichEditOle* pRichEditOle,
    HBITMAP hBitmap) {
    SCODE sc;
    ...
    sc = ::StgCreateDocfileOnILockBytes(
        lpLockBytes, STGM_SHARE_EXCLUSIVE | STGM_CREATE | STGM_READWRITE, 0,
        &pStorage);
    if (sc != S_OK) {
        lpLockBytes = NULL;
    }
...
IOleObject* CImageDataObject::GetOleObject(IOleClientSite* pOleClientSite,
    IStorage* pStorage) {
    SCODE sc;
    IOleObject* pOleObject;
    sc = ::OleCreateStaticFromData(this, IID_IOleObject, OLERENDER_FORMAT,
        &m_fromat, pOleClientSite, pStorage,
        (void**)&pOleObject);
