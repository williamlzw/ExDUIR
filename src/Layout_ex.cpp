        ...
        if (lpfnProc != 0) {
            ...
            if (pLayout != 0) {
                ...
                if (nError == 0) {
                    ...
                    INT nCount =
                        ((LayoutPROC)lpfnProc)(pLayout, LAYOUT_EVENT_GETPROPSCOUNT, NULL, NULL);
                    LPVOID pInfo =
                        (LPVOID)((size_t)Ex_MemAlloc((nCount + (size_t)4) * sizeof(size_t)) +
                                 (size_t)4 * sizeof(size_t));   // padding
                    pLayout->lpLayoutInfo_ = pInfo;
                    ((LayoutPROC)lpfnProc)(pLayout, LAYOUT_EVENT_INITPROPS, NULL, (size_t)pInfo);
                    nCount =
                        ((LayoutPROC)lpfnProc)(pLayout, LAYOUT_EVENT_GETCHILDPROPCOUNT, NULL, NULL);
...
BOOL _layout_table_setinfo(HEXLAYOUT hLayout, INT* aRowHeight, INT cRows, INT* aCellWidth,
                           INT cCells)
{
    INT       nError  = 0;
    layout_s* pLayout = nullptr;
    ...
    if (_handle_validate(hLayout, HT_LAYOUT, (LPVOID*)&pLayout, &nError)) {
        if (pLayout->nType_ == LAYOUT_TYPE_TABLE) {
            ...
            array_s* hArr =
                (array_s*)__get(pInfo, (LAYOUT_PROP_TABLE_ARRAY_ROW - 1) * sizeof(LPVOID));
...
BOOL _layout_setprop(HEXLAYOUT hLayout, INT dwPropID, size_t pvValue)
{
    ...
    if (_handle_validate(hLayout, HT_LAYOUT, (LPVOID*)&pLayout, &nError)) {
        LPVOID pInfo    = pLayout->lpLayoutInfo_;
        LPVOID lpfnProc = pLayout->lpfnProc_;
        if (((LayoutPROC)lpfnProc)(pLayout, LAYOUT_EVENT_CHECKCHILDPROPVALUE, dwPropID, pvValue) ==
            0) {
            if (dwPropID > 0) {
                dwPropID = dwPropID - 1;
            }
...
BOOL _layout_absolute_setedge(HEXLAYOUT hLayout, HEXOBJ hObjChild, INT dwEdge, INT dwType,
                              size_t nValue)
{
    layout_s* pLayout = nullptr;
    INT       nError  = 0;
...
LRESULT CALLBACK __layout_linear_proc(layout_s* pLayout, INT nEvent, WPARAM wParam, LPARAM lParam)
{
    if (nEvent == LAYOUT_EVENT_GETPROPSCOUNT) {
        ...
        if (nSize == LAYOUT_EVENT_CHECKCHILDPROPVALUE) {
            return (lParam < LAYOUT_SUBPROP_LINEAR_ALIGN_FILL ||
                    lParam > LAYOUT_SUBPROP_LINEAR_ALIGN_RIGHT_BOTTOM);
        }
    }
        ...
        BOOL     fVertical = __get(pInfo, (LAYOUT_PROP_LINEAR_DIRECTION - 1) * sizeof(size_t)) ==
                         LAYOUT_PROP_DIRECTION_V;
        ...
        rcClient.bottom =
            rcClient.bottom - __get(pInfo, LAYOUT_PROP_PADDING_BOTTOM * sizeof(size_t));
        SIZE szClient;
        ...
        for (INT i = 1; i <= Array_GetCount(hArr); i++) {
            ...
            if (nDAlign == 0) {
                ...
                rcObj.right =
                    rcObj.right + __get(pInfo, LAYOUT_SUBPROP_MARGIN_RIGHT * sizeof(size_t));
                rcObj.bottom =
                    rcObj.bottom + __get(pInfo, LAYOUT_SUBPROP_MARGIN_BOTTOM * sizeof(size_t));
                arrRect[i - 1] = rcObj;
                arrOrg[i - 1]  = orgFlags;
            }
            if (fVertical) {
        ...
        if (Array_GetCount(hArr) > 0 && arrRect.size() > 0) {
            ...
            if (fVertical) {
                ...
                {
                    w = rcClient.top + (rcClient.bottom - rcClient.top - nSize) / 2 -
                        arrRect[0].top;
                }
                else {
                    w = 0;
                ...
                {
                    w = rcClient.left + (rcClient.right - rcClient.top - nSize) / 2 -
                        arrRect[0].left;
                }
                else {
                    w = 0;
...
LRESULT CALLBACK __layout_flow_proc(layout_s* pLayout, INT nEvent, WPARAM wParam, LPARAM lParam)
{
    if (nEvent == LAYOUT_EVENT_GETPROPSCOUNT) {
        ...
        array_s* hArr      = ((layout_s*)pLayout)->hArrChildrenInfo_;
        BOOL     fVertical = __get(pInfoa, (LAYOUT_PROP_LINEAR_DIRECTION - 1) * sizeof(size_t)) ==
                         LAYOUT_PROP_DIRECTION_V;
        ...
        rcClient.bottom =
            rcClient.bottom - __get(pInfoa, LAYOUT_PROP_PADDING_BOTTOM * sizeof(size_t));
        RECT rcObj{0};
        rcObj.left = rcClient.left;
        rcObj.top  = rcClient.top;
        ...
        for (INT i = 1; i <= Array_GetCount(hArr); i++) {
            ...
            if (fVertical) {
                ...
                rcObj.bottom = rcObj.top + nSize;
                if (rcObj.bottom > rcClient.bottom ||
                    __get(pInfo, LAYOUT_SUBPROP_FLOW_NEW_LINE * sizeof(size_t)) != 0) {
                    rcObj.top    = rcClient.top;
                    rcObj.bottom = rcObj.top + nSize;
...
LRESULT CALLBACK __layout_table_proc(layout_s* pLayout, INT nEvent, WPARAM wParam, LPARAM lParam)
{
    if (nEvent == LAYOUT_EVENT_GETPROPSCOUNT) {
        ...
        Array_Destroy(
            (array_s*)__get((LPVOID)lParam, (LAYOUT_PROP_TABLE_ARRAY_ROW - 1) * sizeof(size_t)));
        Array_Destroy(
            (array_s*)__get((LPVOID)lParam, (LAYOUT_PROP_TABLE_ARRAY_CELL - 1) * sizeof(size_t)));
    }
    else if (nEvent == LAYOUT_EVENT_UPDATE) {
        RECT rcClient{0};
        if (((layout_s*)pLayout)->nBindType_ == HT_OBJECT) {
        ...
        array_s* hArr   = ((layout_s*)pLayout)->hArrChildrenInfo_;
        array_s* hArrRows =
            (array_s*)__get(pInfoa, (LAYOUT_PROP_TABLE_ARRAY_ROW - 1) * sizeof(size_t));
        array_s* hArrCells =
            (array_s*)__get(pInfoa, (LAYOUT_PROP_TABLE_ARRAY_CELL - 1) * sizeof(size_t));
        ...
        if (Array_GetCount(hArrRows) > 0 && Array_GetCount(hArrCells) > 0) {
            ...
            for (INT i = 1; i <= Array_GetCount(hArrRows); i++) {
                ...
                if (rcTmp.bottom < 0) {
                    rcTmp.bottom =
                        (rcClient.bottom - rcClient.top) * (FLOAT)abs(rcTmp.bottom) / 100;
                }
                for (INT j = 1; j <= Array_GetCount(hArrCells); j++) {
                    ...
                    if (rcTmp.right < 0) {
                        rcTmp.right =
                            (rcClient.right - rcClient.left) * (FLOAT)abs(rcTmp.right) / 100;
                    }
        ...
        if (Ex_ObjIsValidate(lParam) && LOWORD(wParam) > 0) {
            ...
            if (hObj != 0 && dwPropID > 0 && pInfoOther != 0) {
                if (dwPropID == LAYOUT_SUBPROP_RELATIVE_LEFT_OF) {
                    nRet = __get(pInfoOther, LAYOUT_SUBPROP_RELATIVE_RIGHT_OF * sizeof(size_t)) !=
                           hObj;
                }
                else if (dwPropID == LAYOUT_SUBPROP_RELATIVE_RIGHT_OF) {
                    nRet =
                        __get(pInfoOther, LAYOUT_SUBPROP_RELATIVE_LEFT_OF * sizeof(size_t)) != hObj;
                }
                else if (dwPropID == LAYOUT_SUBPROP_RELATIVE_TOP_OF) {
                    nRet = __get(pInfoOther, LAYOUT_SUBPROP_RELATIVE_BOTTOM_OF * sizeof(size_t)) !=
                           hObj;
                }
                else if (dwPropID == LAYOUT_SUBPROP_RELATIVE_BOTTOM_OF) {
                    nRet =
                        __get(pInfoOther, LAYOUT_SUBPROP_RELATIVE_TOP_OF * sizeof(size_t)) != hObj;
                }
                else {
                             ...
                             LPARAM lParam)
{
    ...
    rcClient.right =
        rcClient.right - __get(pLayoutInfo, LAYOUT_PROP_PADDING_RIGHT * sizeof(size_t));
    rcClient.bottom =
        rcClient.bottom - __get(pLayoutInfo, LAYOUT_PROP_PADDING_BOTTOM * sizeof(size_t));
    ...
    for (INT i = 1; i <= Array_GetCount(hArrObjs); i++) {
        ...
        LPVOID pPosInfo = Ex_MemAlloc(4 * 3 * sizeof(size_t) + sizeof(size_t) +
                                      sizeof(size_t));   // 多一个放pInfo和orgFlags
        ...
        if (fNoPosInfoH == FALSE) {   // 左边界处理
            ...
            if (__get(pInfo, LAYOUT_SUBPROP_RELATIVE_LEFT_ALIGN_OF * sizeof(size_t)) != 0) {
                ...
                {
                    Infos[2] =
                        rcClient.left + __get(pInfo, LAYOUT_SUBPROP_MARGIN_LEFT * sizeof(size_t));
                }
                else if (dwValue != 0 && (hObjParent == Ex_ObjGetParentEx(dwValue, &hExDUI)) ||
                         hObjParent == hExDUI)   // 同层组件
                {
                    Infos[0] = 0;
                    Infos[2] = dwValue;
                }
                ...
                Infos[2] = rcClient.left + abs(rcClient.right - rcClient.left - rcObj.right) / 2 +
                           __get(pInfo, LAYOUT_SUBPROP_MARGIN_LEFT * sizeof(size_t));
                orgFlags = orgFlags | 4;
            }
            else {
                INT    dwValue = __get(pInfo, LAYOUT_SUBPROP_RELATIVE_RIGHT_OF * sizeof(size_t));
                HEXDUI hExDUI  = 0;
                if (dwValue != 0 && (hObjParent == Ex_ObjGetParentEx(dwValue, &hExDUI)) ||
                    hObjParent == hExDUI)   // 同层组件
                {
                    Infos[0] = 0;
            ...
            if (__get(pInfo, LAYOUT_SUBPROP_RELATIVE_RIGHT_ALIGN_OF * sizeof(size_t)) != 0) {
                ...
                {
                    Infos[2] =
                        rcClient.right + __get(pInfo, LAYOUT_SUBPROP_MARGIN_LEFT * sizeof(size_t));
                }
                else if (dwValue != 0 && (hObjParent == Ex_ObjGetParentEx(dwValue, &hExDUI)) ||
                         hObjParent == hExDUI)   // 同层组件
                {
        ...
        if (fNoPosInfoV == FALSE) {
            ...
            if (__get(pInfo, LAYOUT_SUBPROP_RELATIVE_TOP_ALIGN_OF * sizeof(size_t)) != 0) {
                ...
                Infos[2] = rcClient.top + abs(rcClient.bottom - rcClient.top - rcObj.bottom) / 2 +
                           __get(pInfo, LAYOUT_SUBPROP_MARGIN_TOP * sizeof(size_t));
                orgFlags = orgFlags | 8;
            }
            else {
            ...
            if (__get(pInfo, LAYOUT_SUBPROP_RELATIVE_BOTTOM_ALIGN_OF * sizeof(size_t)) != 0) {
                INT dwValue =
                    __get(pInfo, LAYOUT_SUBPROP_RELATIVE_BOTTOM_ALIGN_OF * sizeof(size_t));
                Infos[1]      = LAYOUT_SUBPROP_RELATIVE_BOTTOM_ALIGN_OF;
                HEXDUI hExDUI = 0;
                if (dwValue == -1)   // 相对父
                {
                    Infos[2] = rcClient.bottom +
                               __get(pInfo, LAYOUT_SUBPROP_MARGIN_TOP * sizeof(size_t)) -
                               __get(pInfo, LAYOUT_SUBPROP_MARGIN_BOTTOM * sizeof(size_t));
                }
                else if (dwValue != 0 && (hObjParent == Ex_ObjGetParentEx(dwValue, &hExDUI)) ||
        ...
        for (INT i = 1; i <= Array_GetCount(hArrObjs); i++) {
            ...
            if (pPosInfo != 0) {
                ...
                if (__get((LPVOID)((size_t)pPosInfo + 3 * sizeof(size_t) * 0), 0) == 0) {
                    ...
                    if (Infos[1] == LAYOUT_SUBPROP_RELATIVE_RIGHT_OF) {
                        ...
                        if (HashTable_Get(hHashPosInfos, Infos[2], (size_t*)&pInfoOther)) {
                            ...
                            {
                                Infos[2] =
                                    __get(pInfoOther, 2 * 3 * sizeof(size_t) + 2 * sizeof(size_t)) +
                                    __get(pInfo, LAYOUT_SUBPROP_MARGIN_LEFT * sizeof(size_t));
                                Infos[0] = 1;
                            }
                        }
                        ...
                        if (HashTable_Get(hHashPosInfos, Infos[2], (size_t*)&pInfoOther)) {
                            ...
                            {
                                Infos[2] =
                                    __get(pInfoOther, 0 * 3 * sizeof(size_t) + 2 * sizeof(size_t)) +
                                    __get(pInfo, LAYOUT_SUBPROP_MARGIN_LEFT * sizeof(size_t));
                                Infos[0] = 1;
                            }
                        }
                        ...
                        {
                            Infos[2] =
                                __get(pPosInfo, 2 * 3 * sizeof(size_t) + 2 * sizeof(size_t)) -
                                Infos[2] -
                                __get(pInfo, LAYOUT_SUBPROP_MARGIN_RIGHT * sizeof(size_t)) -
                                __get(pInfo, LAYOUT_SUBPROP_MARGIN_LEFT * sizeof(size_t));
                            Infos[0] = 1;
                ...
                if (__get((LPVOID)((size_t)pPosInfo + 3 * sizeof(size_t) * 1), 0) == 0) {
                    ...
                    Infos[1] =
                        __get((LPVOID)((size_t)pPosInfo + 3 * sizeof(size_t)), sizeof(size_t));
                    Infos[2] =
                        __get((LPVOID)((size_t)pPosInfo + 3 * sizeof(size_t)), 2 * sizeof(size_t));
                    ...
                    if (Infos[1] == LAYOUT_SUBPROP_RELATIVE_BOTTOM_OF) {
                        ...
                        if (HashTable_Get(hHashPosInfos, Infos[2], (size_t*)&pInfoOther)) {
                            ...
                            {
                                Infos[2] =
                                    __get(pInfoOther, 3 * 3 * sizeof(size_t) + 2 * sizeof(size_t)) +
                                    __get(pInfo, LAYOUT_SUBPROP_MARGIN_TOP * sizeof(size_t));
                                Infos[0] = 1;
                            }
                        }
                        ...
                        if (HashTable_Get(hHashPosInfos, Infos[2], (size_t*)&pInfoOther)) {
                            ...
                            {
                                Infos[2] =
                                    __get(pInfoOther, 1 * 3 * sizeof(size_t) + 2 * sizeof(size_t)) +
                                    __get(pInfo, LAYOUT_SUBPROP_MARGIN_TOP * sizeof(size_t));
                                Infos[0] = 1;
                            }
                        }
                        ...
                        {
                            Infos[2] =
                                __get(pPosInfo, 3 * 3 * sizeof(size_t) + 2 * sizeof(size_t)) -
                                Infos[2] -
                                __get(pInfo, LAYOUT_SUBPROP_MARGIN_BOTTOM * sizeof(size_t)) -
                                __get(pInfo, LAYOUT_SUBPROP_MARGIN_TOP * sizeof(size_t));
                            Infos[0] = 1;
                ...
                if (__get((LPVOID)((size_t)pPosInfo + 3 * sizeof(size_t) * 2), 0) == 0) {
                    ...
                    Infos[1] =
                        __get((LPVOID)((size_t)pPosInfo + 3 * sizeof(size_t) * 2), sizeof(size_t));
                    Infos[2] = __get((LPVOID)((size_t)pPosInfo + 3 * sizeof(size_t) * 2),
                                     2 * sizeof(size_t));
                    ...
                    if (Infos[1] == LAYOUT_SUBPROP_RELATIVE_LEFT_OF) {
                        ...
                        {
                            Infos[2] =
                                __get(pPosInfo, 0 * 3 * sizeof(size_t) + 2 * sizeof(size_t)) +
                                Infos[2] +
                                __get(pInfo, LAYOUT_SUBPROP_MARGIN_LEFT * sizeof(size_t)) +
                                __get(pInfo, LAYOUT_SUBPROP_MARGIN_RIGHT * sizeof(size_t));
                            Infos[0] = 1;
                ...
                if (__get((LPVOID)((size_t)pPosInfo + 3 * sizeof(size_t) * 3), 0) == 0) {
                    ...
                    Infos[1] =
                        __get((LPVOID)((size_t)pPosInfo + 3 * sizeof(size_t) * 3), sizeof(size_t));
                    Infos[2] = __get((LPVOID)((size_t)pPosInfo + 3 * sizeof(size_t) * 3),
                                     2 * sizeof(size_t));
                    ...
                    if (Infos[1] == LAYOUT_SUBPROP_RELATIVE_TOP_OF) {
                        ...
                        {
                            Infos[2] =
                                __get(pPosInfo, 1 * 3 * sizeof(size_t) + 2 * sizeof(size_t)) +
                                Infos[2] +
                                __get(pInfo, LAYOUT_SUBPROP_MARGIN_TOP * sizeof(size_t)) +
                                __get(pInfo, LAYOUT_SUBPROP_MARGIN_BOTTOM * sizeof(size_t));
                            Infos[0] = 1;
                ...
                if (__get(pInfo, 0) != 0 && __get(pInfo, 3 * sizeof(size_t)) != 0 &&
                    __get(pInfo, 6 * sizeof(size_t)) != 0 &&
                    __get(pInfo, 9 * sizeof(size_t)) != 0)   // 已经确定整个组件了
                {
    ...
    for (size_t i = 0; i < hObjs.size(); i++) {
        ...
        if (__get((LPVOID)((size_t)pPosInfo + 0 * 3 * sizeof(size_t)), 0) == 0) {
            ...
            rcTmp.left =
                __get((LPVOID)((size_t)pPosInfo + 0 * 3 * sizeof(size_t)), 2 * sizeof(size_t));
        }
        if (__get((LPVOID)((size_t)pPosInfo + 1 * 3 * sizeof(size_t)), 0) == 0) {
            ...
            rcTmp.top =
                __get((LPVOID)((size_t)pPosInfo + 1 * 3 * sizeof(size_t)), 2 * sizeof(size_t));
        }
        if (__get((LPVOID)((size_t)pPosInfo + 2 * 3 * sizeof(size_t)), 0) == 0) {
            ...
            rcTmp.right =
                __get((LPVOID)((size_t)pPosInfo + 2 * 3 * sizeof(size_t)), 2 * sizeof(size_t));
        }
...
LRESULT CALLBACK __layout_absolute_proc(layout_s* pLayout, INT nEvent, WPARAM wParam, LPARAM lParam)
{
    if (nEvent == LAYOUT_EVENT_GETPROPSCOUNT) {
        ...
        if (nType % 2 == 1 && nType >= LAYOUT_SUBPROP_ABSOLUTE_LEFT &&
            nType <= LAYOUT_SUBPROP_ABSOLUTE_OFFSET_V) {
            if (__get(pInfo, (nType + 1) * sizeof(size_t)) ==
        ...
        if (_handle_validate(hObj, pLayout->nBindType_, (LPVOID*)&pObj, &nError)) {
            *nCount = 0;
            ...
            while (_handle_validate(hObj, HT_OBJECT, (LPVOID*)&pObj2, &nError)) {
                ...
                if ((dwObjClassATOM == 0 || dwObjClassATOM == clsAtom) &&
                    clsAtom != ATOM_SYSBUTTON) {
                    if (_layout_get_child(pLayout, hObj) == 0) {
...
BOOL _layout_absolute_lock(HEXLAYOUT hLayout, HEXOBJ hObjChild, INT tLeft, INT tTop, INT tRight,
                           INT tBottom, INT tWidth, INT tHeight)
{
    layout_s* pLayout = nullptr;
    INT       nError  = 0;
    LPVOID    pInfo   = nullptr;
    if (_handle_validate(hLayout, HT_LAYOUT, (LPVOID*)&pLayout, &nError)) {
        ...
        if (pLayout->nType_ == LAYOUT_TYPE_ABSOLUTE) {
            ...
            rcClient.top  = rcClient.top + __get(pInfo, LAYOUT_PROP_PADDING_TOP * sizeof(size_t));
            rcClient.right =
                rcClient.right - __get(pInfo, LAYOUT_PROP_PADDING_RIGHT * sizeof(size_t));
            rcClient.bottom =
                rcClient.bottom - __get(pInfo, LAYOUT_PROP_PADDING_BOTTOM * sizeof(size_t));
            SIZE szClient{0};
            szClient.cx = rcClient.right - rcClient.left;
