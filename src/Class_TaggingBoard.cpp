...
void _taggingboard_register()
{
	...
	Ex_ObjRegister(wzCls, OBJECT_STYLE_VISIBLE | OBJECT_STYLE_HSCROLL | OBJECT_STYLE_VSCROLL, OBJECT_STYLE_EX_FOCUSABLE | OBJECT_STYLE_EX_COMPOSITED, 0, 20 * sizeof(size_t), 0, 0, _taggingboard_proc);
}
...
LRESULT CALLBACK _taggingboard_proc(HWND hWnd, HEXOBJ hObj, INT uMsg, WPARAM wParam, LPARAM lParam)
{
	if (uMsg == WM_CREATE)
	{
		...
		Ex_ObjSetLong(hObj_scroll, OBJECT_LONG_OBJPROC, (size_t)_taggingboard_OnScrollBarMsg); //改变滚动条回调
...
void _taggingboard_paint(HEXOBJ hObj)
{
	...
	if (Ex_ObjBeginPaint(hObj, &ps))
	{
		...
		if (tagging == 1)
		{
			...
			if (!pointNull)
			{
				_canvas_drawcanvas(ps.hCanvas, canvas, 0, 0, ps.uWidth, ps.uHeight, 0, 0, 255, CANVAS_COMPOSITE_MODE_SRCOVER);
			}
		}
