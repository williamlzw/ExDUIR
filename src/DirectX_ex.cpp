...
BOOL _dx_init(INT *nError)
{
    ...
    *nError = D3D11CreateDevice(NULL, D3D_DRIVER_TYPE_HARDWARE, NULL, D3D11_CREATE_DEVICE_BGRA_SUPPORT, NULL, 0, D3D11_SDK_VERSION, &pD3DDevice, NULL, NULL);
    ...
    {
       /* *nError = D2D1CreateFactory(D2D1_FACTORY_TYPE_MULTI_THREADED, __uuidof(ID2D1Factory1), (LPVOID *)&(g_Ri.pD2Dfactory));*/
...
//下面为设置启用调试层，会在IDE输出窗口内显示,消息框和菜单项目会有彩色外框
        ...
        //创建工厂
        //*nError = D2D1CreateFactory(D2D1_FACTORY_TYPE_MULTI_THREADED, options, & g_Ri.pD2Dfactory);
