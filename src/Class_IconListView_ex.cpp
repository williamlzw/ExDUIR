...
LRESULT CALLBACK _iconlistview_proc(HWND hWnd, HEXOBJ hObj, INT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    else if (uMsg == LISTVIEW_MESSAGE_INSERTITEM)   // 插入表项 lParam
                                                    // 为表项信息,wParam为是否立即重画,返回索引
    {
        size_t ret = 0;
        if (lParam != 0) {
        ...
        return ret;
             ...
             LISTVIEW_MESSAGE_DELETEITEM)   // 删除表项 wParam为是否立即重画，lParam为删除的索引
    {
    ...
    {
        array_s* pArray = (array_s*)Ex_ObjGetLong(hObj, ICONLISTVIEW_LONG_ITEMARRAY);
        if (Ex_ObjSendMessage(hObj, LISTVIEW_MESSAGE_GETITEMCOUNT, 0, 0) !=
            Array_GetCount(pArray))   // 如果表项数不同则更新表项数
        {
    ...
    else if (uMsg == LISTVIEW_MESSAGE_GETIMAGELIST)   // 获取图片组
    {
        return Ex_ObjGetLong(hObj, ICONLISTVIEW_LONG_HIMAGELIST);
             ...
             uMsg == WM_LBUTTONUP)   // 当鼠标左键按下放开时 更新控件按下状态并重画
    {
        if ((Ex_ObjGetLong(hObj, OBJECT_LONG_STYLE) & ICONLISTVIEW_STYLE_BUTTON) !=
            0)   // 当前得是按钮列表状态
        {
...
{
    EX_ICONLISTVIEW_ITEMINFO* pItemInfo =
        (EX_ICONLISTVIEW_ITEMINFO*)Ex_MemAlloc(sizeof(EX_ICONLISTVIEW_ITEMINFO));
...
BOOL _iconlistview_ondrawitem(HEXOBJ hObj, EX_CUSTOMDRAW* cdr)
{
    ...
    if (Array_GetCount(pArray) >= cdr->iItem && cdr->iItem > 0)   // 如果索引在范围内
    {
        ...
        EX_ICONLISTVIEW_ITEMINFO* pItemInfo =
            (EX_ICONLISTVIEW_ITEMINFO*)Array_GetMember(pArray, cdr->iItem);
        INT nWidthIcon  = 0;
        INT nHeightIcon = 0;
        ...
        {
            if ((cdr->dwState & STATE_HOVER) != 0 &&
                (Ex_ObjGetUIState(hObj) & STATE_DOWN) != 0)   // 当当前表项是悬浮且左键按下时
            {
                hBrush = _brush_create(ExRGB2ARGB(16777215, 100));
            }
            else if ((cdr->dwState & STATE_HOVER) != 0)   // 当当前表项是悬浮时
            {
    ...
    return (cdr->dwStyle & ICONLISTVIEW_STYLE_BUTTON) !=
           0;   // 如果不是按钮列表状态，则表项选中、悬浮的状态交给列表框绘制
}
