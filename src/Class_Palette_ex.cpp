...
void _palette_register()
{
	...
	Ex_ObjRegister(L"Palette", EOS_VISIBLE | EOS_BORDER, EOS_EX_FOCUSABLE, DT_LEFT, cbObjExtra, LoadCursor(0, IDC_HAND), ECVF_CANVASANTIALIAS, _palette_proc);
}
...
LRESULT CALLBACK _palette_proc(HWND hWnd, HEXOBJ hObj, INT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	if (_handle_validate(hObj, HT_OBJECT, (LPVOID*)&pObj, &nError))
	{
		if (uMsg == WM_CREATE)
		{
			_struct_createfromaddr(pObj, offsetof(obj_s, dwOwnerData_), sizeof(palette_s), &nError);
			palette_s* pOwner = (palette_s*)_obj_pOwner(pObj);
			pOwner->hObj = hObj;
		...
		else if (uMsg == WM_LBUTTONUP)
		{
			...
			else
			{
				...
				HEXDUI hExBox = Ex_DUIBindWindowEx(hWndBox, 0, EWS_ESCEXIT | EWS_NOINHERITBKG | EWS_NOCAPTIONTOPMOST | EWS_POPUPWINDOW, (size_t)pOwner, _palette_onwndmsgproc);
				if (hExBox)
				{
...
LRESULT CALLBACK _palette_onwndmsgproc(HWND hWnd, HEXDUI hExDUI, INT uMsg, WPARAM wParam, LPARAM lParam, LRESULT* lpResult)
{
	if (uMsg == WM_DESTROY)
	{
		palette_s* pOwner = (palette_s*)Ex_DUIGetLong(hExDUI, EWL_LPARAM);
		pOwner->nProcessTime = GetTickCount64();
	}
	...
	else if (uMsg == PTM_COLORCHANGE) {
		palette_s* pOwner = (palette_s*)Ex_DUIGetLong(hExDUI, EWL_LPARAM);
		EXARGB dwColor = _palette_getcolor(wParam);
		Ex_ObjSetColor(pOwner->hObj, COLOR_EX_BACKGROUND, ExRGB2ARGB(dwColor, 255), TRUE);
		Ex_ObjDispatchNotify(pOwner->hObj, PTN_COLORCHANGE, 0, (LPARAM)dwColor);
	...
	if (uMsg == WM_NOTIFY)
	{
		...
		if (hObj == ni.hObjFrom) {
			...
			else if (ni.nCode == NM_CUSTOMDRAW)
			{
				...
				_brush_setcolor(hBrush, ExRGB2ARGB(_palette_getcolor(cd.iItem), 255));
			...
			else if (ni.nCode == LVN_ITEMCHANGED)
			{
				...
				PostMessageW(hWnd, PTM_COLORCHANGE, ni.wParam, 0);
				*lpResult = 1;
				return 1;
			}
		}
