...
void _rollmenu_register()
{
	Ex_ObjRegister(L"RollMenu", OBJECT_STYLE_VISIBLE | OBJECT_STYLE_VSCROLL,
		...
		15 * sizeof(size_t), NULL, 0, _rollmenu_proc);
}
...
LRESULT CALLBACK _rollmenu_proc(HWND hWnd, HEXOBJ hObj, INT uMsg, WPARAM wParam, LPARAM lParam)
{
	if (uMsg == WM_CREATE)
	{
		Ex_ObjSetLong(hObj, ROLLMENU_LONG_GTC, (LONG_PTR)ExRGB2ARGB(16777215, 255));//分组标题颜色
		...
		Ex_ObjSetLong(hObj, ROLLMENU_LONG_ISTC, (LONG_PTR)ExRGB2ARGB(16777215, 200));//子项选中标题色
		Ex_ObjSetColor(hObj, COLOR_EX_TEXT_NORMAL, ExRGB2ARGB(16777215, 255), FALSE);//子项正常标题颜色
	...
	else if (uMsg == WM_MOUSEMOVE)
	{
		...
		for (int i = 1; i <= count; i++)
		{
			...
			if (!data->expand)
			{
				array_s* itemarr = (array_s*)*(size_t*)((BYTE*)data + sizeof(EX_ROLLMENU_DATA));// data->arr;
	...
	else if (uMsg == ROLLMENU_MESSAGE_ADDGROUP)//添加分组  wParam :索引(从1开始)  lParam: ROLLMENU_DATA * 指针
	{
		EX_ROLLMENU_DATA* data = (EX_ROLLMENU_DATA*)Ex_MemAlloc(sizeof(EX_ROLLMENU_DATA) + sizeof(array_s*));
	...
	else if (uMsg == ROLLMENU_MESSAGE_ADDITEM)//添加项目  wParam :分组索引(从1开始)  lParam: ROLLMENU_ITEM * 指针
	{
		array_s* arr = (array_s*)Ex_ObjGetLong(hObj, ROLLMENU_LONG_ARR);
		if (wParam == 0)
		{
	...
	else if (uMsg == ROLLMENU_MESSAGE_DELITEM)//删除子项  wParam :分组索引(从1开始)  lParam:子项索引(从1开始)
	{
		array_s* arr = (array_s*)Ex_ObjGetLong(hObj, ROLLMENU_LONG_ARR);
		if (wParam == 0)
		{
	...
	else if (uMsg == ROLLMENU_MESSAGE_SETEXPAND)//设置分组状态(展开/收缩)  wParam :分组索引(从1开始)  lParam: 状态(BOOL)
	{
		array_s* arr = (array_s*)Ex_ObjGetLong(hObj, ROLLMENU_LONG_ARR);
		if (wParam == 0)
		{
	...
	else if (uMsg == ROLLMENU_MESSAGE_GETSEL) //wParam: int* 分组索引(从1开始)  lParam : int* 子项索引(从1开始)  return:子项标题
	{
	...
	else if (uMsg == ROLLMENU_MESSAGE_SETSEL)//wParam: 分组索引(从1开始)  lParam : 子项索引(从1开始)
	{
		array_s* arr = (array_s*)Ex_ObjGetLong(hObj, ROLLMENU_LONG_ARR);
		int count = Array_GetCount(arr);
		if (wParam > count)
		...
		if (count > 0)
		{
			for (int i = 1; i <= count; i++)
			{
				...
				if (CFG == i)
				{
					...
					_canvas_fillrect(ps.hCanvas, brush, 0, endhight - offset - GH, ps.uWidth, endhight - offset);
				}
				if (data->title)//画分组标题
				{
					_canvas_drawtext(ps.hCanvas, font, GTCcolor, data->title,
						   -1, -1,
						   data->left * g_Li.DpiX,
						   endhight - offset - GH,
						   ps.uWidth,
				...
				if (!data->expand)
				{
					...
					{
						_canvas_drawimagerect(ps.hCanvas, data->stateico.eicon,
					...
					if (itemcount > 0)
					{
						for (int n = 1; n <= itemcount; n++)
						{
							...
							if (iDvalue > 0 && iDvalue - IH <= ps.uHeight)// -IH 画末尾不完全显示的子项  
							{
								page++;
								if (CLI == n && CLG == i)
								{
									_brush_setcolor(brush, MLBC);//子项点燃色  MLBC
									_canvas_fillrect(ps.hCanvas, brush, 0, iDvalue - IH, ps.uWidth, iDvalue);
								}
								EX_ROLLMENU_ITEM* itemdata = (EX_ROLLMENU_ITEM*)Array_GetMember(itemarr, n);
								if (CLI == n && CLG == i)
								{
									tempcolor = ExRGB2ARGB(16777215, 255);//修改鼠标悬浮子项的标题色
								}
								...
								if (itemdata->title)//子项标题
								{
									_canvas_drawtext(ps.hCanvas, font, tempcolor, itemdata->title,
										-1, -1, itemdata->left * g_Li.DpiX, iDvalue - IH, ps.uWidth, iDvalue);
								}
								//--画子项状态(是否选中)图标
								if (CSI == n && CSG == i)//选中子项
								{
								...
								else
								{
									...
									{
										_canvas_drawimagerect(ps.hCanvas, itemdata->stateico.eicon,
											itemdata->stateico.rc.left * g_Li.DpiX,
											iDvalue - IH + itemdata->stateico.rc.top * g_Li.DpiY,
											itemdata->stateico.rc.right * g_Li.DpiX,
											iDvalue - IH + itemdata->stateico.rc.bottom * g_Li.DpiY, 255);
								...
								if (itemdata->extraico.icon)//画子项的附加图标
								{
									_canvas_drawimagerect(ps.hCanvas,
											...
											iDvalue - IH + itemdata->extraico.rc.bottom * g_Li.DpiY, 255);
								}
							}
						}
					}
				...
				else
				{
					...
					{
						_canvas_drawimagerect(ps.hCanvas, data->stateico.sicon,
							data->stateico.rc.left* g_Li.DpiX,
							endhight - offset - GH + data->stateico.rc.top * g_Li.DpiY,
							data->stateico.rc.right* g_Li.DpiX,
							endhight - offset - GH + data->stateico.rc.bottom * g_Li.DpiY, 255);
			...
			Ex_ObjScrollSetInfo(hObj, SCROLLBAR_TYPE_VERT, SIF_PAGE | SIF_RANGE, 0, vmax, page, 0, 1);
		}
