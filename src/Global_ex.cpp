...
BOOL Ex_Init(HINSTANCE hInstance, DWORD dwGlobalFlags, HCURSOR hDefaultCursor,
  ...
  g_Li.atomClassName =
      Ex_WndRegisterClass(lpszDefaultClassName, NULL, NULL, NULL);
  ...
  g_Li.aryColorsAtom = {ATOM_BACKGROUND_COLOR,
                        ATOM_COLOR_BACKGROUND,
                        ATOM_BORDER_COLOR,
                        ATOM_COLOR_BORDER,
                        ATOM_COLOR,
                        ATOM_COLOR_NORMAL,
  ...
  g_Li.aryColorsOffset = {
      offsetof(obj_s, crBackground_), offsetof(obj_s, crBackground_),
      offsetof(obj_s, crBorder_),     offsetof(obj_s, crBorder_),
      offsetof(obj_s, crNormal_),     offsetof(obj_s, crNormal_),
      offsetof(obj_s, crHover_),      offsetof(obj_s, crDown_),
      offsetof(obj_s, crFocus_),      offsetof(obj_s, crChecked_),
      offsetof(obj_s, crSelected_),   offsetof(obj_s, crHot_),
  ...
  g_Li.hHookMsgBox =
      SetWindowsHookExW(WH_CBT, (HOOKPROC)_hook_proc, 0, GetCurrentThreadId());
...
LPVOID Ex_AllocBuffer(size_t dwSize) { return Ex_MemAlloc(dwSize); }

BOOL Ex_FreeBuffer(LPVOID lpBuffer) { return Ex_MemFree(lpBuffer); }
      ...
      if (lpMem) {
        ...
        if (uType == IMAGE_ICON || uType == IMAGE_CURSOR) {
          ...
          if (s >= nIndex) {
            ...
            LPVOID offset = (LPVOID)((size_t)lpData + 6 +
                                     (nIndex - 1) * sizeof(ICONDIRENTRY));
            LPVOID tmp = lpMem;
            if (!bIcon) {
            ...
            RtlMoveMemory(tmp,
                          (LPVOID)((size_t)lpData +
                                   ((ICONDIRENTRY*)offset)->dwImageOffset),
            ...
            HICON hicon = CreateIconFromResourceEx(
                (PBYTE)lpMem, ((ICONDIRENTRY*)offset)->dwBytesInRes, bIcon,
                196608, b1, b2, 0);
          ...
          if (CreateStreamOnHGlobal(hMem, FALSE, &lpStream)) {
            ...
            if (OleLoadPicture(lpStream, dwLen, TRUE, IID_IPicture, &lpObj) ==
                0) {
              OLE_HANDLE hImg = 0;
    ...
    if (hr == 0) {
      ...
      if (hr == 0) {
        ...
        pConverter->Initialize(
            (IWICBitmap*)pSource, GUID_WICPixelFormat32bppPBGRA,
...
BOOL Ex_ReadResSource(WORD lpname, LPCWSTR lpType, std::vector<CHAR>* retData) {
  ...
  if (lpname) {
    ...
    /*
     * 在资源使用完毕后我们可以不需要使用
     * UnlockResource和FreeResource来手动地释放资源
...
DOUBLE Ex_DUIGetSystemDpi() {
  ...
  DOUBLE dpiA = (DOUBLE)GetDeviceCaps(desktopDc, DESKTOPHORZRES) /
