...
IDWriteFontFileLoader* Resource_FontFileLoader::instance_(
	new(std::nothrow) Resource_FontFileLoader()
);
...
)
{
	*fontFileStream = NULL;
	...
	if (fontFileReferenceKeySize != 8 || fontFileReferenceKeySize == 0)
		return E_FAIL;
...
void STDMETHODCALLTYPE Resource_FontFileStream::ReleaseFileFragment(
	void* fragmentContext
)
{
}

HRESULT STDMETHODCALLTYPE Resource_FontFileStream::GetFileSize(
	OUT UINT64* fileSize
)
{
	*fileSize = m_size;
...
HRESULT STDMETHODCALLTYPE Resource_FontFileStream::GetLastWriteTime(
...
Resource_FontFileEnumerator::Resource_FontFileEnumerator(int nextIndex, IDWriteFontFile* currentFile, void const* collectionKey, int collectionKeySize, IDWriteFactory* factory)
{
	m_refCount = 0;
...
HRESULT STDMETHODCALLTYPE Resource_FontFileEnumerator::MoveNext(OUT BOOL* hasCurrentFile)
{
	...
	*hasCurrentFile = false;
	...
	if (m_nextIndex < m_collectionKeySize)
	{
		...
		HRESULT hr = m_factory->CreateCustomFontFileReference((void*)fliepath, 8, Resource_FontFileLoader::GetLoader(), &m_currentFile);
...
HRESULT STDMETHODCALLTYPE Resource_FontFileEnumerator::GetCurrentFontFile(OUT IDWriteFontFile** fontFile)
{
...
HRESULT STDMETHODCALLTYPE Resource_FontCollectionLoader::QueryInterface(REFIID iid, void** ppvObject)
{
...
HRESULT STDMETHODCALLTYPE Resource_FontCollectionLoader::CreateEnumeratorFromKey(IDWriteFactory* factory, void const* collectionKey, UINT32 collectionKeySize, OUT IDWriteFontFileEnumerator** fontFileEnumerator)
{
	Resource_FontFileEnumerator* pFontFileEnumerator = new Resource_FontFileEnumerator
	(
		0, 0, collectionKey, collectionKeySize / 8, factory
	);
...
MFFontContext::MFFontContext(IDWriteFactory* pFactory) : hr_(S_FALSE), hr2_(S_FALSE), g_dwriteFactory(pFactory)
{
}
...
)
{
	*result = NULL;
	...
	hr = g_dwriteFactory->CreateCustomFontCollection(
		Resource_FontCollectionLoader::GetLoader(),
		collectionKey,
		collectionKeySize,
		result
	);
