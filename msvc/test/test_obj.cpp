...
void test_label(HWND hWnd)
{
	...
	Ex_ObjSetFontFromFamily(hObj_label4, L"宋体", 14, FONT_STYLE_BOLD, FALSE);
...
LRESULT CALLBACK OnEditButtonEvent(HEXOBJ hObj, INT nID, INT nCode, WPARAM wParam, LPARAM lParam)
{
	...
	if (hEdit)
	{
		...
		else if (nID == 226) //替换文本
		{
			...
			Ex_ObjSendMessage(hEdit, EDIT_MESSAGE_SETTEXTEX, (WPARAM)&textformat, (LPARAM)L"选中替换为我");
		}
...
void test_edit(HWND hWnd)
{
	...
	Ex_ObjSendMessage(hObj_edit1, EDIT_MESSAGE_SETCUEBANNER, ExARGB(0, 0, 0, 100), (LPARAM)L"背景图片编辑框");
	std::vector<CHAR> imgdata;
	...
	HEXOBJ hObj_edit2 = Ex_ObjCreateEx(OBJECT_STYLE_EX_FOCUSABLE | OBJECT_STYLE_EX_COMPOSITED, L"edit", L"测试密码输入编辑框", OBJECT_STYLE_VISIBLE | EDIT_STYLE_USEPASSWORD, 10, 70, 150, 30, m_hExDuiEdit, 0, DT_SINGLELINE, 0, 0, NULL);
...
void test_custombkg(HWND hWnd)
{
	...
	Ex_ObjCreateEx(OBJECT_STYLE_EX_TOPMOST, L"sysbutton", L"", OBJECT_STYLE_VISIBLE | WINDOW_STYLE_BUTTON_CLOSE, 140, 8, 30, 30, hExDui_custombkg, 300, 0, 0, 0, NULL);
...
LRESULT CALLBACK OnNavButtonCheckEvent(HEXOBJ hObj, INT nID, INT nCode, WPARAM wParam, LPARAM lParam)
{
	if (lParam != 0)
	{
		...
		_layout_setprop(m_hLayout, LAYOUT_PROP_PAGE_CURRENT, m_nCurIndex + 1);
...
void test_navbutton(HWND hWnd)
{
	...
	for (INT i = 0; i < 4; i++)
	{
		...
		Ex_ObjSetLong(m_hNavBtn[i], OBJECT_LONG_LPARAM, i);
...
void test_treelistview(HWND hWnd)
{
	...
	HEXOBJ hObj_treeview = Ex_ObjCreateEx(OBJECT_STYLE_EX_FOCUSABLE, L"treeview", L"fasdf", OBJECT_STYLE_BORDER | OBJECT_STYLE_VISIBLE | OBJECT_STYLE_HSCROLL | OBJECT_STYLE_VSCROLL, 50, 50, 250, 250, hExDui_treelistview, 0, DT_VCENTER, 0, 0, NULL);
...
void test_chromium(HWND hParent)
{
	...
	//Ex_ObjSendMessage(m_hObjChromium, CEFBROWSER_MESSAGE_LOADURL, 0, (LPARAM)L"d:/ExDUIR/msvc/test/res/cefjs.html");
...
LRESULT CALLBACK OnMediaBtnEnevt(HEXOBJ hObj, INT nID, INT nCode, WPARAM wParam, LPARAM lParam)
{
	...
	{
		Ex_ObjSendMessage(m_hObjMedia, MEDIAPLAYER_MESSAGE_STATE_PLAY, 0, (LPARAM)L"./res/test.mp4");//可不带file:///前缀
	}
	else if (hObj == m_hObjMediaButton2)
	{
		Ex_ObjSendMessage(m_hObjMedia, MEDIAPLAYER_MESSAGE_STATE_PAUSE, 0, 0);
	}
	else if (hObj == m_hObjMediaButton3)
	{
		Ex_ObjSendMessage(m_hObjMedia, MEDIAPLAYER_MESSAGE_STATE_CONTINUE, 0, 0);
	}
	else if (hObj == m_hObjMediaButton4)
	{
		Ex_ObjSendMessage(m_hObjMedia, MEDIAPLAYER_MESSAGE_STATE_STOP, 0, 0);
	}
	else if (hObj == m_hObjMediaButton5)
	{
		Ex_ObjSendMessage(m_hObjMedia, MEDIAPLAYER_MESSAGE_SET_POSITION, 0, 20);
	}
	return 0;
}
