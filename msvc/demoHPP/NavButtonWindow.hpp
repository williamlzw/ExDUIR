...
public:
	void CreateNavButtonWindow(HWND pOwner)
	{
		...
		for (INT i = 0; i < 4; i++)
		{
			...
			auto file = L"../demo/res/navbtn/大图标" + std::to_wstring(i + 1) + L".png";
			ExImage img = ExImage(file);
			m_navbuttons[i].SetIcon(img);
			m_navbuttons[i].SetColorTextNormal(ExRGB2ARGB(16777215, 255));
			ExImage img1 = ExImage(L"../demo/res/navbtn/顶部按钮背景C.png");
			ExImage img2 = ExImage(L"../demo/res/navbtn/顶部按钮背景D.png");
