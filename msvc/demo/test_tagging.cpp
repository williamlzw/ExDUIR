...
LRESULT CALLBACK OnTaggingButtonEvent(HEXOBJ hObj, INT nID, INT nCode, WPARAM wParam, LPARAM lParam)
{
    if (nID == 1010) {
        ...
        float* pImgLeftOffset = (float*)Ex_ObjSendMessage(hObj_taggingBoard,
            TAGGINGBOARD_MESSAGE_GET_IMG_LEFT_OFFSET, 0, 0);
        auto  offsetLeft = *pImgLeftOffset;
        float* pImgTopOffset = (float*)Ex_ObjSendMessage(hObj_taggingBoard,
            TAGGINGBOARD_MESSAGE_GET_IMG_TOP_OFFSET, 0, 0);
        auto  offsetTop = *pImgTopOffset;
        float* scalePtr = (float*)Ex_ObjSendMessage(hObj_taggingBoard, 
            TAGGINGBOARD_MESSAGE_GET_IMG_SCALE, 0, 0);
