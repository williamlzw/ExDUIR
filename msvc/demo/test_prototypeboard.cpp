...
void test_prototypeboard(HWND hParent)
{
	...
	auto label = Ex_ObjCreateEx(-1, L"static", L"1.按住CTRL+鼠标滚轮可缩放画布\r\n"
		...
		-1, 1280, 30, 100, 300, hExDui_prototypeboard, 0, DT_WORDBREAK, 0, 0, 0);

	auto hButton1 = Ex_ObjCreateEx(-1, L"Button", L"拖动模式", -1, 1280, 350, 100, 30, hExDui_prototypeboard, 100, -1, 0, 0, 0);
	Ex_ObjHandleEvent(hButton1, NM_CLICK, OnPrototypeBoardButtonEvent);

	auto hButton2 = Ex_ObjCreateEx(-1, L"Button", L"选择模式", -1, 1280, 390, 100, 30, hExDui_prototypeboard, 101, -1, 0, 0, 0);
	Ex_ObjHandleEvent(hButton2, NM_CLICK, OnPrototypeBoardButtonEvent);

	auto hButton3 = Ex_ObjCreateEx(-1, L"Button", L"绘制模式", -1, 1280, 430, 100, 30, hExDui_prototypeboard, 102, -1, 0, 0, 0);
	Ex_ObjHandleEvent(hButton3, NM_CLICK, OnPrototypeBoardButtonEvent);

	auto hButton4 = Ex_ObjCreateEx(-1, L"Button", L"添加矩形形状", -1, 1280, 470, 100, 30, hExDui_prototypeboard, 103, -1, 0, 0, 0);
	Ex_ObjHandleEvent(hButton4, NM_CLICK, OnPrototypeBoardButtonEvent);

	auto hButton5 = Ex_ObjCreateEx(-1, L"Button", L"添加直线形状", -1, 1280, 510, 100, 30, hExDui_prototypeboard, 104, -1, 0, 0, 0);
	Ex_ObjHandleEvent(hButton5, NM_CLICK, OnPrototypeBoardButtonEvent);

	auto hButton6 = Ex_ObjCreateEx(-1, L"Button", L"添加椭圆形状", -1, 1280, 550, 100, 30, hExDui_prototypeboard, 105, -1, 0, 0, 0);
	Ex_ObjHandleEvent(hButton6, NM_CLICK, OnPrototypeBoardButtonEvent);

	auto hButton7 = Ex_ObjCreateEx(-1, L"Button", L"添加图片形状", -1, 1280, 590, 100, 30, hExDui_prototypeboard, 106, -1, 0, 0, 0);
	Ex_ObjHandleEvent(hButton7, NM_CLICK, OnPrototypeBoardButtonEvent);

	auto hButton8 = Ex_ObjCreateEx(-1, L"Button", L"插入图片", -1, 1280, 630, 100, 30, hExDui_prototypeboard, 107, -1, 0, 0, 0);
	Ex_ObjHandleEvent(hButton8, NM_CLICK, OnPrototypeBoardButtonEvent);

	auto hButton9 = Ex_ObjCreateEx(-1, L"Button", L"添加文本形状", -1, 1280, 670, 100, 30, hExDui_prototypeboard, 108, -1, 0, 0, 0);
	Ex_ObjHandleEvent(hButton9, NM_CLICK, OnPrototypeBoardButtonEvent);

	auto hButton10 = Ex_ObjCreateEx(-1, L"Button", L"插入文本", -1, 1280, 710, 100, 30, hExDui_prototypeboard, 109, -1, 0, 0, 0);
